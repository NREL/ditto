...
class Writer(abstract_writer):
    ...
    **Output file names:**
        ...
        for i in self.m.models:
            ...
            if isinstance(i, Node):
                ...
                if ((hasattr(i, 'name') and i.name is not None) and
                        (hasattr(i, 'positions') and i.positions is not None and len(i.positions) > 0)):
    ...
    def transformer(self):
        ...
        """
        obj_dict = {'ID': [], 'W1Bus A': [], 'W1Bus B': [], 'W1Bus C': [], 'W1V (kV)': [],
                         'W1S_base (kVA)': [], 'W1R (pu)': [], 'W1Conn. type': [], 'W2Bus A': [],
                         'W2Bus B': [], 'W2Bus C': [], 'W2V (kV)': [], 'W2S_base (kVA)': [],
                         'W2R (pu)': [], 'W2Conn. type': [], 'X (pu)': [], 'Tap A': [], 'Tap B': [],
                         'Tap C': [], 'Lowest Tap': [], 'Highest Tap': [], 'Min Range (%)': [],
                         'Max Range (%)': []}
            ...
            if len(i.windings) >= 2:
                ...
                if hasattr(i, 'windings') and i.windings is not None:
                    ...
                    for winding_num, winding in enumerate(i.windings):
                        ...
                        if hasattr(winding, 'connection_type') and winding.connection_type is not None:
                            if winding.connection_type == 'Y':
                                ...
                                self.logger.error('Unsupported type of connection {conn} for transformer {name}'.format(
                    ...
                    if len(np.unique(N_phases)) != 1:
                        self.logger.error(
                            'Did not find the same number of phases accross windings of transformer {name}'.format(name=i.name))
        ...
        df4 = df4[
            ['ID', 'W1Bus A', 'W1Bus B', 'W1Bus C', 'W1V (kV)', 'W1S_base (kVA)', 'W1R (pu)', 'W1Conn. type', 'W2Bus A',
             'W2Bus B', 'W2Bus C', 'W2V (kV)', 'W2S_base (kVA)', 'W2R (pu)', 'W2Conn. type', 'X (pu)', 'Tap A', 'Tap B',
             'Tap C', 'Lowest Tap', 'Highest Tap', 'Min Range (%)', 'Max Range (%)']]
    ...
    def source(self):
        ...
        obj_dict = {'bus A': [], 'bus B': [], 'bus C': [], 'ID': [], 'V (kV)': [], 'Angle (deg)': [],
                         'SCL_1 (MVA)': [], 'SCL_3 (MVA)': [], 'R_pos (ohm)': [], 'X_pos (ohm)': [],
                         'R_zero (ohm)': [], 'X_zero (ohm)': []}
        ...
        df7 = df7[
            ['bus A', 'bus B', 'bus C', 'ID', 'V (kV)', 'Angle (deg)', 'SCL_1 (MVA)', 'SCL_3 (MVA)', 'R_pos (ohm)',
             'X_pos (ohm)', 'R_zero (ohm)', 'X_zero (ohm)']]
    ...
    def load(self):
        ...
        """
        obj_dict = {'Bus A': [], 'Bus B': [], 'Bus C': [], 'ID': [], 'Type': [], 'P_a (kW)': [], 'Q_a (kVAr)': [],
             'P_b (kW)': [], 'Q_b (kVAr)': [], 'P_c (kW)': [], 'Q_c (kVAr)': [], 'V (kV)': [],
             'Bandwidth (pu)': [], 'Conn. type': [], 'K_z': [], 'K_i': [], 'K_p': [], 'Status': [],
             'Use initial voltage?': []}
                    ...
                    if hasattr(j, 'phase') and j.phase is not None:
                        ...
                        if hasattr(j, 'use_zip') and j.use_zip is not None:
                            if j.use_zip == 1:  # This means that all the required values are not None
                                print('    current_fraction_{phase} {cf};\n'.format(phase=j.phase, cf=(
                                j.ppercentcurrent + j.qpercentcurrent)))
                                print('    current_pf_{phase} {cpf};\n'.format(phase=j.phase, cpf=(
                                j.ppercentcurrent / (j.ppercentcurrent + j.qpercentcurrent))))
                                print('    power_fraction_{phase} {pf};\n'.format(phase=j.phase, pf=(
                                j.ppercentpower + j.qpercentpower)))
                                print('    power_pf_{phase} {ppf};\n'.format(phase=j.phase, ppf=(
                                j.ppercentpower / (j.ppercentpower + j.qpercentpower))))
                                print('    impedance_fraction_{phase} {iff};\n'.format(phase=j.phase, iff=(
                                j.ppercentimpedance + j.qpercentimpedance)))
                                print('    impedance_pf_{phase} {ipf};\n'.format(phase=j.phase, ipf=(
                                j.ppercentimpedance / (j.ppercentimpedance + j.qpercentimpedance))))
                                ...
                                if hasattr(j, 'p') and j.p is not None and hasattr(j, 'q') and j.q is not None and hasattr(
                                        j, 'phase') and j.phase is not None:
        ...
        df3 = df3[
            ['Bus A', 'Bus B', 'Bus C', 'ID', 'Type', 'P_a (kW)', 'Q_a (kVAr)', 'P_b (kW)', 'Q_b (kVAr)', 'P_c (kW)',
             'Q_c (kVAr)', 'V (kV)', 'Bandwidth (pu)', 'Conn. type', 'K_z', 'K_i', 'K_p', 'Status',
             'Use initial voltage?']]
