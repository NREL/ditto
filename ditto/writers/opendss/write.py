                        ...
                        for cnt,winding in enumerate(i.windings):
                            ...
                            if hasattr(winding, 'connection_type') and winding.connection_type is not None:
                                if winding.connection_type=='Y':
                                    ...
                                    self.logger.error('Unsupported type of connection {conn} for transformer {name}'.format(conn=winding.connection_type, name=i.name))
                            ...
                            if hasattr(winding, 'phase_windings') and winding.phase_windings is not None:
                                ...
                                if self.write_taps and hasattr(winding.phase_windings[0], 'tap_position') and winding.phase_windings[0].tap_position is not None:
                        ...
                        if hasattr(i, 'reactances') and i.reactances is not None:
                            ...
                            if isinstance(i.reactances, list):
                                if len(i.reactances)!=1:
                                    self.logger.error('Number of reactances incorrect for transformer {name}. Expected 1, got {N}'.format(name=i.name, N=len(i.reactances)))
                                else:
        ...
        for i in model.models:
            if isinstance(i,Timeseries):
                if hasattr(i,'data_location') and i.data_location is not None and os.path.isfile(i.data_location) and (i.scale_factor is None or i.scale_factor == 1):
                    ...
                    fp.write('New loadshape.{filename} npts= {npoints} interval=1 mult = (file={data_location})\n\n'.format(filename=filename,npoints=npoints,data_location=i.data_location))
                            ...
                            if phase_load.use_zip:
                                ...
                                if ((hasattr(i,'ppercentimpedance') and i.ppercentimpedance is not None) and
                                    ...
                                    (hasattr(i,'qpercentpower')     and i.qpercentpower     is not None)):
                                        ...
                                        fp.write(' model=8 ZIPV=[%.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f]'%(i.ppercentimpedance,
                                                                                                            i.ppercentcurrent,
                                                                                                            i.ppercentpower,
                                                                                                            i.qpercentimpedance,
                                                                                                            i.qpercentcurrent,
                ...
                if i.name in self.compensator:
                    if 'X' in self.compensator[i.name]:
                        if len(self.compensator[i.name]['X'])==1:
                            ...
                            self.logger.warning('''Compensator_x not the same for all windings of transformer {name}.
                                                   Using the first value for regControl {name2}.'''.format(name=i.connected_transformer,
                                                                                                           name2=i.name))
                            fp.write(' X={x}'.format(x=list(self.compensator[i.name]['X'])[0]))
                ...
                if i.name in self.compensator:
                    if 'R' in self.compensator[i.name]:
                        if len(self.compensator[i.name]['R'])==1:
                            ...
                            self.logger.warning('''Compensator_r not the same for all windings of transformer {name}.
                                                   Using the first value for regControl {name2}.'''.format(name=i.connected_transformer,
                                                                                                           name2=i.name))
                            fp.write(' R={r}'.format(r=list(self.compensator[i.name]['R'])[0]))
...
'''
        ...
        #Emergency ampacity
        if hasattr(line,'wires') and line.wires is not None and len(line.wires)>0 and hasattr(line.wires[0], 'ampacity_emergency') and line.wires[0].ampacity_emergency is not None:
