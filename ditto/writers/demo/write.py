        ...
        for i in model.models:
            if isinstance(i, Line):
                if hasattr(i,'from_element') and hasattr(i,'to_element'):
                    fp.write('{frome} {toe}:\n'.format(frome=i.from_element.upper(),toe= i.to_element.upper()))
                    ...
                    if hasattr(i,'impedance_matrix') and i.impedance_matrix is not None:
                        all_z = [ [ complex(0,0) for posi in range(3) ] for posj in range(3)]
...
#                                dic['z{one}{two}'.format(one=phase_map[j_val], two=phase_map[k_val])] = impedance
                    ...
                    logger.warning('Line missing from and to elements. Nothing written for line {}'.format(i.name))  
