...
'''
        ...
        if self.verbose: print('Writing the load file...')
        self.write_load_file(model, **kwargs)

        #Writing the network file
        if self.verbose: print('Writing the network file...')
        self.write_network_file(model, **kwargs)

        #Writing the equipment file
        if self.verbose: print('Writing the equipment file...')
        ...
        with open(output_file, 'w') as f:
            ...
            for i in model.models:
                ...
                if isinstance(i, Regulator):
                    ...
                    if hasattr(i, 'windings') and i.windings is not None:
                        if (len(i.windings)>=2 and
                            ...
                            if i.windings[0].nominal_voltage != i.windings[1].nominal_voltage:
                                ...
                                CONN = ''
                                try:
                                    new_transformer_line += ',' + self.transformer_connection_configuration_mapping(
                                        i.windings[0].connection_type, i.windings[1].connection_type
                                    )
                                ...
                                new_transformer_object_line += '{type},{kva},{kvllprim},{kvllsec},{Z1},{Z0},{XR},{XR0},{Conn},{WindingType}'.format(
                                    type=TYPE, kva=KVA, kvllprim=KVLLprim, kvllsec=KVLLsec, Conn=CONN, Z1=Z1, Z0=Z0, XR=XR, XR0=XR0, WindingType=2
                                )
                    ...
                    if hasattr(i, 'positions') and i.positions is not None:
                        try:
                            new_regulator_string += ',' + str(i.positions[0].lon)
                    ...
                    new_regulator_object_line = '{kva},{band},{ct},{pt},{Type},{KVLN},{MaxBuck},{MaxBoost},{Taps},{Reversible}'.format(
                        kva=_KVA, band=_band, ct=_CT, pt=_PT, Type=0, KVLN=_KVLN, MaxBuck=10, MaxBoost=10, Taps=0, Reversible=0
                    )
                    ...
                    if not found:
                        ...
                        new_regulator_string += ',regulator_' + str(ID_reg) + ',' + new_section_ID
                    ...
                    new_regulator_string += ',M,10,10,T,{RsetA},{RsetB},{RsetC},{XsetA},{XsetB},{XsetC},0,0,0,0'.format(
                        RsetA=_Rset['A'], RsetB=_Rset['B'], RsetC=_Rset['C'], XsetA=_Xset['A'], XsetB=_Xset['B'], XsetC=_Xset['C']
                    )
                    ...
                    if hasattr(i, 'windings') and i.windings is not None:
                        ...
                        #
                            ...
                            new_transformer_line += ',{Location},{tertiarynodeID},{PrimaryFixedTapSetting},{SecondaryFixedTapSetting},{ConnectionStatus}'.format(
                                Location='M', tertiarynodeID=0, PrimaryFixedTapSetting=0, SecondaryFixedTapSetting=0, ConnectionStatus=0
                            )
            ...
            if len(two_windings_transformer_string_list) > 0:
                f.write('\n[TRANSFORMER SETTING]\n')
                f.write(
                    'FORMAT_TRANSFORMERSETTING=SectionID,CoordX,CoordY,Conn,PhaseON,EqID,DeviceNumber,Location,PrimTap,SecondaryTap,ODPrimPh,ConnectionStatus,Tap\n'
                )
...
'''
        ...
        with open(output_file, 'w') as f:
            ...
            if len(self.linecodes) > 0:
                f.write('\n[LINE UNBALANCED]\n')
                f.write(
                    'FORMAT_LINEUNBALANCED=ID,Ra,Rb,Rc,Xa,Xb,Xc,MutualResistanceAB,MutualResistanceBC,MutualResistanceCA,MutualReactanceAB,MutualReactanceBC,MutualReactanceCA,CondID_A,CondID_B,CondID_C,SpacingID,Ba,Bb,Bc,AmpsA,AmpsB,AmpsC,UserDefinedImpedances\n'
                )

                for ID, data in self.linecodes.items():
                    f.write('line_' + str(ID))
                    for key in [
                        'RA', 'RB', 'RC', 'XA', 'XB', 'XC', 'MutualResistanceAB', 'MutualResistanceBC', 'MutualResistanceCA', 'MutualReactanceAB',
                        'MutualReactanceBC', 'MutualReactanceCA', 'CondID_A', 'CondID_B', 'CondID_C', 'SpacingID', 'Ba', 'Bb', 'Bc', 'AmpsA', 'AmpsB',
                        'AmpsC', 'UserDefinedImpedances'
                    ]:
                        if key in data:
                            f.write(',' + str(data[key]))
                        else:
