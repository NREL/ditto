...
class Modifier:
    ...
    def delete_element(self, model,obj):
        for attr in obj.traits():
            ...
            if class_name == 'List':
                if getattr(obj,attr) is None or getattr(obj,attr) is None or len(getattr(obj,attr))==0 or attr == 'reactances' or attr == 'phases' or attr == 'impedance_matrix' or attr == 'capacitance_matrix':#Reactances (PowerTransformer) and phases (Node) are a special case of lists that aren't classes TODO: Add type checking rather than looking at the attributes
                    continue
    ...
    def copy(self, model, obj):
        ...
        for attr in obj.traits():
            class_name = str(type(obj.traits()[attr])).strip("<>'").split('.')[-1]
            if class_name == 'List':
                if getattr(obj,attr) is None or len(getattr(obj,attr)) ==0 or attr == 'reactances' or attr == 'phases' or attr == 'impedance_matrix' or attr == 'capacitance_matrix': #Reactances (PowerTransformer) and phases (Node) are a special case of lists that aren't classes
    ...
    def set_attributes(self, model_1,obj_1, obj_2, overwrite=True):
         for attr in obj_2.traits(): # Iterate through all attributes in obj_2. These should be the same traits as obj_1 assuming the precondition
            ...
            if class_name == 'List':
                phase_order = {'A':0, 'B':1, 'C':2, 'N': 3} #Should only have to deal with 3 phases.
#                   
                ...
                for i in range(len(list_1)):
                    if not(hasattr(list_1[0],'phase') and list_1[0].phase is not None):
                        has_phases = False
                for i in range(len(list_2)):
                    if not(hasattr(list_2[0],'phase') and list_2[0].phase is not None):
                        has_phases = False
                if has_phases and len(list_1)>0 and len(list_2)>0:
                    ...
                    while list_1_idx < len(list_1) and list_2_idx < len(list_2):
                        ...
                        if list_1_phase == list_2_phase:
                            self.set_attributes(model_1, list_1[list_1_idx], list_2[list_2_idx], overwrite)
                            list_1_idx = list_1_idx+1
                            list_2_idx = list_2_idx+1
                        elif list_1_phase < list_2_phase:
                            list_1_idx = list_1_idx + 1 # e.g. obj_1 = (A, B, C) and obj_2 = (B). We don't update this phase
    ...
    def delete(self, model_1, model_2):
        ...
        for obj_name in model_2.model_names:
            if obj_name in model_1.model_names: # Find the corresponding object in model_1
                ...
                obj_1 = model_1.model_names[obj_name]
                self.delete_element(model_1,obj_1) #model_1 updated automatically since passing by reference
        model_1.set_names()
        return model_1
