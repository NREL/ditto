...
'''
    ...
    def set_missing_coords_recur(self, recur_nodes = None):
        ''' Identify nodes that don't have coordinates set and set them to be the
            ...
            If no adjacent nodes have positional values continue to compute recursively
        '''
        if recur_nodes == []:
            return
        if recur_nodes is None:
            recur_nodes = []
            for i in self.model.models:
                if hasattr(i,'positions') and (i.positions is None or len(i.positions) == 0 or i.positions[0].lat ==0 or i.positions[0].long ==0) and (hasattr(i,'name') and i.name is not None):
                    if i.name in self.G.graph.nodes(): # Only find coords of missing nodes - not edges
                        recur_nodes.append(i) # Should be passing the reference to the node

        recur_nodes_size = len(recur_nodes)
                    
        next_recur = []
        for i in recur_nodes:
            adj_lats_longs = []
            for j_name in self.G.graph.neighbors(i.name):
                j = self.model[j_name]
            ...
            if len(adj_lats_longs) == 0:
                ...
                i.positions = [computed_pos]
        if len(next_recur) == len(recur_nodes):
            for i in recur_nodes:
                logger.warning('Unable to compute coordinates for {}'.format(i.name))
                print('Unable to compute coordinates for {}'.format(i.name))
            return
        self.set_missing_coords_recur(recur_nodes=next_recur)
