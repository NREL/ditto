                    ...
                    if not skip:
                        ...
                        for down_elt in downstream_elts:
                            if down_elt.substation_name is not None and len(down_elt.substation_name)!=0:
                                raise ValueError('Substation name for element {name} was already set at {_previous}. Trying to overwrite with {_next}'.format(name=down_elt.name, _previous=down_elt.substation_name, _next=elt.name))
                            else:
                                down_elt.substation_name=elt.name

                            if down_elt.feeder_name is not None and len(down_elt.feeder_name)!=0:
                                raise ValueError('Feeder name for element {name} was already set at {_previous}. Trying to overwrite with {_next}'.format(name=down_elt.name, _previous=down_elt.feeder_name, _next='Feeder_'+elt.name))
                            else:
