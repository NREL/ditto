"""
Created on Wed Sep 12 3:18:42 2019
@author: kduwadi
"""
...
class Writer:
    ...
    # capacitance must be in 'nanofarad', resistance, reactance in 'ohm'
    data_dict = {
            'xlpe3x300': {'r0': 0.12819167775767903, 'r1': 0.1282, 'x0':0.10555612525190466, 'x1':0.10555751316061705, 'c0':509.97756, 'c1': 510.0,'normamps': 360},
            'xlpe4x240': {'r0': 0.1599912042937184, 'r1': 0.16, 'x0':0.06911606174395343, 'x1': 0.06911503837897544, 'c0':849.9625999999998, 'c1': 849.9999999999999, 'normamps': 320},
            'xlpe4x120': {'r0': 0.32398398829371844, 'r1': 0.324, 'x0':0.06912022778128678, 'x1': 0.06911503837897544, 'c0':649.9713999999999, 'c1': 649.9999999999999,'normamps': 215},
            'xlpe4x25': {'r0': 1.5399300054647325, 'r1': 1.54, 'x0':0.0879998439650316, 'x1': 0.08796459430051422, 'c0':399.98240000000004, 'c1': 400.00000000000006,'normamps': 95},
            'xlpe4x16': {'r0': 2.4498898856599016, 'r1': 2.45, 'x0':0.09116441481521126, 'x1': 0.091106186954104, 'c0':199.99120000000002, 'c1': 200.00000000000003,'normamps': 73},
            'xlpe2x16': {'r0': 2.4498901426314577, 'r1': 2.45, 'x0':0.0810489315714995, 'x1': 0.08099025860954487, 'c0':149.9934, 'c1': 149.99999999999997,'normamps': 78}
        }
    ...
    def __init__(self, network, path, project_name,penetration_level,PV_cap,rootpath,mapping_dict):
        ...
        self.FeederHead = network.SS_node
    ...
    def __Create_Line_section(self):
        ...
        for node1, node2 in self.__nxGraph.edges():
            ...
            self.__nxGraph[node1][node2]['Name']  = self.__ModifyName(self.__projectname) + edgeData['Type'] + '_' + str(index)
            if edgeData['Type'] in ['HT line','LT line','Service line','EHT line']:            
                ...
                if edgeData['Type'] == 'LT line':
                    new_name = self.__ModifyName(edgeData['Name']) +'_1'
                    self.Lines[new_name] = dssDict
                    new_name = self.__ModifyName(edgeData['Name']) +'_2'
                    self.Lines[new_name] = dssDict
            
            elif edgeData['Type'] in ['HT cable','LT cable']:
                Linecode = edgeData['Cable type phase'] + str(edgeData['Cable size phase']) +'_' + str(edgeData['num_of_cond'])
                dssDict = {}
                dssDict =self.__define_line_dict(node1,node2,dssDict)  
                dssDict['length'] = edgeData['Length']
                dssDict['units'] = 'm'
                dssDict['Linecode'] = self.__ModifyName(Linecode)
                dssDict['enabled'] = 'True'
                self.Lines[self.__ModifyName(edgeData['Name'])] =  dssDict
                if edgeData['Type'] == 'LT cable':
                    new_name = self.__ModifyName(edgeData['Name']) +'_1'
                    self.Lines[new_name] = dssDict
            
            elif edgeData['Type'] == 'DTs' or edgeData['Type'] == 'PTs':
                if edgeData['Type'] == 'PTs':
                    self.metername = self.__nxGraph[node1][node2]['Name']
                    if self.__nxGraph.node[node1]['Type'] == 'EHTnode' and self.__nxGraph.node[node2]['Type'] == 'HTnode':
                        xfmrpropname = edgeData['Type'] +'-'+ str(self.__nxGraph[node1][node2][self.mapping_dict["PT_cap"]]*1000) + '-'+ '33-11'
                        dssDict = {
                            'buses' : '[{0} {1}]'.format(self.__ModifyName(node1) +'.1.2.3', self.__ModifyName(node2) + '.1.2.3.0'),
                            'Xfmrcode' : xfmrpropname,
                            'leadlag'    : 'lead',
                        }
                    elif self.__nxGraph.node[node2]['Type'] == 'EHTnode' and self.__nxGraph.node[node1]['Type'] == 'HTnode':
                        xfmrpropname = edgeData['Type'] +'-'+ str(self.__nxGraph[node1][node2][self.mapping_dict["PT_cap"]]*1000) + '-'+ '33-11'
                        dssDict = {
                    ...
                    elif self.__nxGraph.node[node1]['Type'] == 'XHTnode' and self.__nxGraph.node[node2]['Type'] == 'EHTnode':
                        xfmrpropname = edgeData['Type'] +'-'+ str(self.__nxGraph[node1][node2][self.mapping_dict["PT_cap"]]*1000) + '-'+ '110-33'
                        dssDict = {
                            'buses' : '[{0} {1}]'.format(self.__ModifyName(node1) +'.1.2.3', self.__ModifyName(node2) + '.1.2.3.0'),
                            'Xfmrcode' : xfmrpropname,
                            'leadlag'    : 'lead',
                        }
                    elif self.__nxGraph.node[node2]['Type'] == 'XHTnode' and self.__nxGraph.node[node1]['Type'] == 'EHTnode':
                        xfmrpropname = edgeData['Type'] +'-'+ str(self.__nxGraph[node1][node2][self.mapping_dict["PT_cap"]]*1000) + '-'+ '110-33'
                        dssDict = {
                            'buses' : '[{0} {1}]'.format(self.__ModifyName(node2) +'.1.2.3', self.__ModifyName(node1) + '.1.2.3.0'),
                            'Xfmrcode' : xfmrpropname,
                    ...
                    if self.__nxGraph.node[node1]['Type'] == 'HTnode':
                        xfmrpropname = edgeData['Type'] +'-'+ str(self.__nxGraph[node1][node2][self.mapping_dict["DT_cap"]]) + '-'+ '11-0.44'
                        dssDict = {
                        'buses' : '[{0} {1}]'.format(self.__ModifyName(node1) +'.1.2.3', self.__ModifyName(node2) + '.1.2.3.0'),
                        'Xfmrcode' : xfmrpropname,
                        'leadlag'    : 'lead',
    ...
    def __create_xfmr_codes(self):
        ...
        for i in range(D):
            dssDict = {
                        'Phases'     : self.mapping_dict["Transformer_properties"]["Phases"],
                        'Windings'   : self.mapping_dict["Transformer_properties"]["windings"],
                        'conns'      : '[{} {}]'.format(self.mapping_dict["SS_conn"][0],self.mapping_dict["SS_conn"][1]) if list_of_unique_codes[i].split('-')[0] == 'PTs' else '[{} {}]'.format(self.mapping_dict["Transformer_properties"]["connection"][0],self.mapping_dict["Transformer_properties"]["connection"][1]),
                        '%R'         : self.mapping_dict["Transformer_properties"]["%R"],
                        'Xhl'        : self.mapping_dict["Transformer_properties"]["Xhl"],
                        'KVAs'       : '[{} {}]'.format(list_of_unique_codes[i].split('-')[1],list_of_unique_codes[i].split('-')[1]),
                        'kvs'        : '[{} {}]'.format(list_of_unique_codes[i].split('-')[2],list_of_unique_codes[i].split('-')[3]),
    ...
    def __Create_line_code(self):
        list_of_line_codes  =[]
        for keys, values in self.Lines.items():
            if 'Linecode' in values:
                list_of_line_codes.append(values['Linecode'])
        
        unique_linecode= np.unique(list_of_line_codes)
        print(unique_linecode)
        D = len(unique_linecode)
        self.linecode = {}

        for i in range(D):
            dssDict = {
            'nphases': 1 if unique_linecode[i].split('_')[1] == '2' else 3,
            'cable': self.data_dict[unique_linecode[i].split('_')[0]],
            'units': 'km',
            }
            self.linecode[self.__ModifyName(unique_linecode[i])] = dssDict
        self.__toDSS('linecode',self.linecode)
    ...
    def __Create_line_geometry(self):
        ...
        for i in range(D):
            ...
            if nphase == 1:
                dssDict = {
                'nconds'  : nphase+1, 'nphases' : nphase,'reduce'  : 'yes', 'cond1': {'cond'   : 1, 'wire': phase_cond, 'x' : x[2], 'h': h[2], 'units': 'm'}, 'cond2': {'cond': 2, 
                'wire' : neutral_cond,'x' : x[3],'h' : h[3],'units' : 'm'},
                }
            
            elif nphase == 2:
                dssDict = {
                'nconds'  : nphase+1, 'nphases' : nphase+1,'reduce'  : 'n', 'cond1': {'cond' : 1, 'wire' : phase_cond,'x' : x[1],'h' : h[1],'units'  : 'm'}, 'cond2' : {'cond'  : 2,
                'wire' : phase_cond,'x' : x[2],'h' : h[2],'units' : 'm'}, 'cond3': {'cond': 3,'wire': phase_cond,'x':x[3],'h': h[3],'units': 'm'},
                }
            else:
                 dssDict = {
                'nconds'  : nphase+1, 'nphases' : nphase, 'reduce'  : 'yes','cond1': {'cond'   : 1, 'wire'    : phase_cond,'x' : x[0],'h' : h[0],'units' : 'm'},'cond2': {'cond'   : 2,
                'wire'    : phase_cond,'x' : x[1],'h' : h[1],'units': 'm'},'cond3': {'cond' : 3,'wire': phase_cond,'x' : x[2],'h'  : h[2],'units' : 'm'},
                'cond4': {'cond'   : 4, 'wire'  : neutral_cond,'x' : x[3],'h' : h[3],'units'  : 'm'},
                }
        ...
        for node in self.__nxGraph.nodes():
            ...
            if 'loads' in nodedata:
                for keys, values in nodedata['loads'].items():
                    if values['phase'] == self.mapping_dict["three_phase_con"] and values['voltage'] == self.mapping_dict["HT_voltage_sec"]:
                        bus1code = '.1.2.3'
                    else:
                        if values['phase'] == self.mapping_dict["three_phase_con"]:
                            bus1code = '.1.2.3.0'
                        else:
                            bus1code = '.' + self.__Phase2Num[values['phase']] + '.0'
                    dssDict = {
                        'phases' : 3 if values['phase'] == self.mapping_dict["three_phase_con"] else 1,
                        'bus1' : self.__ModifyName(node) + bus1code,
                        'kv'   : values['voltage'],
                        'kw'   : values['load'],
                        'pf'   : values['pf'],
                        'yearly' : values['con_type']
                    }
    ...
    def __PV_scenario_generation_mode(self,penetration_level, PV_cap):
        ...
        for keys, values in self.__Load.items():
            if values['kv'] != self.mapping_dict["HT_voltage_sec"]:
                dict_of_LT_loads[keys] = values
                list_of_customers.append(keys)
        
        capacity_list = []
        kwlist = []
        for keys in list_of_customers:
            capacity_list.append(self.__LOADTEC[keys]*1000/(100*365*24*self.yearly_cap_factor))
            kwlist.append(self.__Load[keys]['kw']*1000)

        data_dict = {'PV-cap':capacity_list,'peak-cap':kwlist}
        df = pd.DataFrame.from_dict(data_dict)
        df.to_csv(self.__projectname+'.csv')
    ...
    def __generate_PVShape(self):
        ...
        cons_type = ["agricultural","residential","commercial","industrial"]
        for con in cons_type:
            where_to=  self.__whichpath + '/'+ con + '.csv'
            which_csv = self._rootpath+ '\\Load_multipliers_csvs\\'+ con + '.csv'
            shutil.copy(which_csv, where_to)

        # copy yearly voltage profile and paste
        where_voltage = self._rootpath+ '\\Yearly_voltage_profile\\'+ self.__projectname + '.csv'
        where_to = where_to=  self.__whichpath + '/voltageprofile.csv'

        shutil.copy(where_voltage,where_to)
        
        return
    
    def __create_load_shape(self):
        self.__loadshape = {}
        shapes=['agricultural','commercial','residential','industrial','solarmult']
    ...
    def __analyze_PV_capacity_factor(self):
        ...
        # output_file('Bokeh_plots/'+'monthy_PV_capacity_factor_curve.html')
        # p = figure(x_range=capacity_factor_X, plot_height=250, title = 'PV capacity factor by month')

        # p.vbar(x =capacity_factor_X, top=capacity_factor_Y, width=0.9)
    ...
    def __CreateCircuit(self,CircuitName):
        Z1 = [0.001,0.001]
        Z0 = [0.001,0.001]
        print('Writing File - ' + self.__ModifyName(CircuitName) + '.dss')
        file = open(self.__whichpath + '/' + self.__ModifyName(CircuitName) + '.dss', 'w')
        file.write('clear\n\n')
        file.write('New loadshape.voltage_profile npts=35040 mInterval=15 mult=(file=voltageprofile.csv)\n')
        file.write('New circuit.' + self.__ModifyName(CircuitName).lower() + '\n')
        file.write('~ basekv= 110.0 pu=1.0 phases=3 Z1={0} Z0= {1} bus1={2} yearly=voltage_profile\n'.format(Z1,Z0,self.__FeederNode+'.1.2.3'))
        #file.write('~ MVAsc3=20 MVASC1=21 \n\n'.lower())
        self.__Files.append('wiredata.dss')
        self.__Files.append('linegeometry.dss')
        self.__Files.append('linecode.dss')
        ...
        file.write('Set voltagebases={}\n\n'.format([self.mapping_dict["XHT_voltage"], self.mapping_dict["HT_voltage"], self.mapping_dict["HT_voltage_sec"], self.mapping_dict["LT_voltage"], self.mapping_dict["LT_phase_voltage"]]))  
