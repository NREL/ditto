...
#        inputfile = open(os.path.join(dew_models_dir,modelfile),'r')
                ...
                while True:
                    ...
                    if row_node1[0] == '$CMP,' and row_node1[1] == node_path:
                        if row_node1[18] == '16,':
                            ...
                            voltage_node = float(PTSUB_DPHABKV[int(row_node1[6][:-1])]
                                                 ) * 1000 #multiplied by 1000 to match with ditto, ditto dividing by 1000
                            break
                        else:
                        ...
                        while True:
                            ...
                            if row_pt2[:9] == '$CMPADDPT' and row_pt1[1] == '48,':
                                try:
                                    ...
                                    pass
                                try:
                                    api_regulator.ct_prim = float(PTINST_DSECDRATA[PTINST_IPTROW.index(float(row_pt1[2][:-1]))]
                                                                  ) * float(row_pt1[7][:-1])
                                except AttributeError:
                                    pass
                            ...
                            if w == 0:
                                try:
                                    if len(ph1) == 1:
                                        ...
                                        windings[w].nominal_voltage = float(PTXFRM_DPRIKV[int(PTXFRM_IPTROW.index(iptr_wdg))]
                                                                            ) * 10**3 * pow(3, 0.5) / vmut
                            ...
                            if w == 1:
                                try:
                                    if len(ph1) == 1:
                                        ...
                                        windings[w].nominal_voltage = float(PTXFRM_DSECKV[int(PTXFRM_IPTROW.index(iptr_wdg))]
                                                                            ) * 10**3 * pow(3, 0.5) / vmut
                                    windings[w].rated_power = float(PTXFRM_DNOMKVA[int(PTXFRM_IPTROW.index(iptr_wdg))]) * 10**3 / nreg
                                    if num_windings == 3:
        ...
        # Transformers Start
                    ...
                    while True:
                        ...
                        if row_tf[:13] == '$CMP_NEWDATA1':
                            break
                    try:
                        api_transformer.emergency_power = float(PTXFRM_DFAVLTMRATKVA[int(PTXFRM_IPTROW.index(iptr_wdg))]
                                                                ) * 10**3 #DiTTo in volt ampere
                    except:
                        pass
        ...
        # Wire Data
            if entries[0] == '$CMP,' and (
                entries[18] == '1,' or entries[18] == '65,' or entries[18] == '129,' or entries[18] == '258,' or entries[18] == '2,'
                or entries[18] == '6,'
            ):
                    ...
                    while True:
                        ...
                        if row_wr[:7] == '$CMPCHN':
                            ...
                            if PTLINESPC_TMUTSPC[int(row_wr1[4][:-1])] == 0.0 or PTLINESPC_TMUTSPC[int(row_wr1[4][:-1])] == 1.0:
                                if pw >= num_ph:
                                    wires[pw].X = float(PTLINESPC_DXNEU[int(row_wr1[4][:-1])]
                                                        ) * 0.3048 - float(PTLINESPC_DXPH1ORR1[int(row_wr1[4][:-1])]) * 0.3048
                                    wires[pw].Y = float(PTLINESPC_DYNEU[int(row_wr1[4][:-1])]
                                                        ) * 0.3048 - float(PTLINESPC_DYPH1ORX1[int(row_wr1[4][:-1])]) * 0.3048
                                else:
                                    if pw == 0:
                                    ...
                                    if pw == 1:
                                        wires[pw].X = float(PTLINESPC_DXPH2ORR0[int(row_wr1[4][:-1])]
                                                            ) * 0.3048 - float(PTLINESPC_DXPH1ORR1[int(row_wr1[4][:-1])]) * 0.3048
                                        wires[pw].Y = float(PTLINESPC_DYPH2ORX0[int(row_wr1[4][:-1])]
                                                            ) * 0.3048 - float(PTLINESPC_DYPH1ORX1[int(row_wr1[4][:-1])]) * 0.3048
                                    if pw == 2:
                                        wires[pw].X = float(PTLINESPC_DXPH3ORY0[int(row_wr1[4][:-1])]
                                                            ) * 0.3048 - float(PTLINESPC_DXPH1ORR1[int(row_wr1[4][:-1])]) * 0.3048
                                        wires[pw].Y = float(PTLINESPC_DYPH3ORY1[int(row_wr1[4][:-1])]
                                                            ) * 0.3048 - float(PTLINESPC_DYPH1ORX1[int(row_wr1[4][:-1])]) * 0.3048
                        ...
                        if int(entries[5][:-1]) == 43 or int(entries[5][:-1]) == 44 or int(entries[5][:-1]) == 45 or int(entries[5][:-1]) == 46:
                            if (PTLINESPC_TMUTSPC[int(row_ug1[4][:-1])] == 0.0 or PTLINESPC_TMUTSPC[int(row_ug1[4][:-1])] == 1.0):
                                ...
                                if PTCABCOND_TCONCENTNEU[int(row_ug1[5][:-1])] == 1.0: #CONCENTRIC NEUTRAL
                                    ...
                                    cond_res5 = 0
                                    cond_res6 = 0
                                    cond_res4S = ((7.9385e8) * 0.3048 * float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])])
                                                  ) / (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])]) * Tape_Thick * 1000)
                                    cond_res5S = ((7.9385e8) * 0.3048 * float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])])
                                                  ) / (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])]) * Tape_Thick * 1000)
                                    cond_res6S = ((7.9385e8) * 0.3048 * float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])])
                                                  ) / (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])]) * Tape_Thick * 1000)
                                    sheild_thick1 = Tape_Thick
                                    sheild_thick2 = Tape_Thick
                                    sheild_thick3 = Tape_Thick
                                    sheild_dia1 = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])]) + 2 * Tape_Thick)
                                    #recheck this
                                    sheild_dia2 = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])]) + 2 * Tape_Thick)
                                    #recheck this
                                    sheild_dia3 = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])]) + 2 * Tape_Thick)
                                    #recheck this
                                    nue_strands4 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                    nue_strands5 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                    nue_strands6 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                    ...
                                    if row_ug1[6] == '-1,':
                                        ...
                                        sheild_dia4 = (
                                            float(PTCABCOND_DINSULSUL[int(row_ug1[6][:-1])]) + float(PTCABCOND_DDIANEUSTRNDSUL[int(row_ug1[6][:-1])])
                                        )
                                        #recheck this
                                ...
                                dis = (
                                    [
                                        [0.0, D12, D13, D14, D15, D16, D17], [D21, 0.0, D23, D24, D25, D26, D27], [D31, D32, 0.0, D34, D35, D36, D37],
                                        [D41, D42, D43, 0.0, D45, D46, D47], [D51, D52, D53, D54, 0.0, D56, D57], [D61, D62, D63, D64, D65, 0.0, D67],
                                ...
                                if PTCABCOND_TCONCENTNEU[int(row_ug1[5][:-1])] == 1.0: #Concentric Neutral
                                            ...
                                            if i_cn == j_cn:
                                                if (i_cn > 2 and i_cn != 6):
                                                    if GMRc[i_cn] == 0.0:
                                                        ...
                                                        Z[i_cn][j_cn] = complex(
                                                            Freq_Coeff.real + Rc[i_cn], Freq_Coeff.imag * (math.log(1.0 / GMRc[i_cn]) + Freq_addit)
                                                        )
                                                else:
                                                    if GMR[i_cn] == 0.0:
                                                        Z[i_cn][j_cn] = complex(0.0, 0.0)
                                                        ...
                                                        Z[i_cn][j_cn] = complex(
                                                            Freq_Coeff.real + cond_res[i_cn],
                                                            Freq_Coeff.imag * (math.log(1.0 / GMR[i_cn]) + Freq_addit)
                                                        )
                                            else:
                                                if dis[i_cn][j_cn] == 0.0:
                                                    ...
                                                    Z[i_cn][j_cn] = complex(
                                                        Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / dis[i_cn][j_cn]) + Freq_addit)
                                                    )
                                    ...
                                    if 'A' in ph_w:
                                        if cond_dia1 == 0 or R14 == 0 or nue_strands4 == 0:
                                            ...
                                            den_aph = math.log(R14 / (cond_dia1 / 24.0)
                                                               ) - (1.0 / nue_strands4) * math.log((nue_strands4) * cond_dia4 / 24.0 / R14)
                                            if den_aph == 0:
                                                can = 0
                                            else:
                                    ...
                                    if 'B' in ph_w:
                                        if cond_dia2 == 0 or R25 == 0 or nue_strands5 == 0:
                                            ...
                                            den_bph = math.log(R25 / (cond_dia2 / 24.0)
                                                               ) - (1.0 / nue_strands5) * math.log((nue_strands5) * cond_dia5 / 24.0 / R25)
                                            if den_bph == 0:
                                                cbn = 0
                                            else:
                                    ...
                                    if 'C' in ph_w:
                                        if cond_dia3 == 0 or R36 == 0 or nue_strands6 == 0:
                                            ...
                                            den_cph = math.log(R36 / (cond_dia3 / 24.0)
                                                               ) - (1.0 / nue_strands6) * math.log((nue_strands6) * cond_dia6 / 24.0 / R36)
                                            if den_cph == 0:
                                                ccn = 0
                                            else:
                                            ...
                                            if i_cn == j_cn:
                                                if (i_cn > 2 and i_cn != 6):
                                                    if GMRS[i_cn] == 0.0:
                                                        ...
                                                        Z[i_cn][j_cn] = complex(
                                                            Freq_Coeff.real + shld_res[i_cn],
                                                            Freq_Coeff.imag * (math.log(1.0 / GMRS[i_cn]) + Freq_addit)
                                                        )
                                                elif i_cn == 6:
                                                    if 'A' in ph_w:
                                                        Z[i_cn][j_cn] = complex(
                                                            Freq_Coeff.real + cond_res4S, Freq_Coeff.imag * (math.log(1.0 / GMR4S) + Freq_addit)
                                                        )
                                                    elif 'B' in ph_w:
                                                        Z[i_cn][j_cn] = complex(
                                                            Freq_Coeff.real + cond_res5S, Freq_Coeff.imag * (math.log(1.0 / GMR5S) + Freq_addit)
                                                        )
                                                    elif 'C' in ph_w:
                                                        Z[i_cn][j_cn] = complex(
                                                            Freq_Coeff.real + cond_res6S, Freq_Coeff.imag * (math.log(1.0 / GMR6S) + Freq_addit)
                                                        )
                                                else:
                                                    if GMRS == 0:
                                                        Z[i_cn][j_cn] = complex(0.0)
                                                ...
                                                if ((i_cn == 0 and j_cn == 3) or (i_cn == 1 and j_cn == 4) or (i_cn == 2 and j_cn == 5)):
                                                    if GMRS == 0:
                                                        ...
                                                        Z[i_cn][j_cn] = complex(
                                                            Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / GMRS[i_cn]) + Freq_addit)
                                                        )
                                                else:
                                                    if dis[i_cn][j_cn] == 0:
                                                        Z[i_cn][j_cn] = complex(0.0)
                                    ...
                                    zints = np.matrix(
                                        [
                                            [Z[0][3], Z[0][4], Z[0][5], Z[0][6]], [Z[1][3], Z[1][4], Z[1][5], Z[1][6]],
                                            [Z[2][3], Z[2][4], Z[2][5], Z[2][6]]
                                        ]
                                    )
                                    znjts = np.matrix(
                                        [
                                            [Z[3][0], Z[3][1], Z[3][2]], [Z[4][0], Z[4][1], Z[4][2]], [Z[5][0], Z[5][1], Z[5][2]],
                                            [Z[6][0], Z[6][1], Z[6][2]]
                                        ]
                                    )
                                    znnts = np.matrix(
                                        [
                                            [Z[3][3], Z[3][4], Z[3][5], Z[3][6]], [Z[4][3], Z[4][4], Z[4][5], Z[4][6]],
                                            [Z[5][3], Z[5][4], Z[5][5], Z[5][6]], [Z[6][3], Z[6][4], Z[6][5], Z[6][6]]
                                        ]
                                    )
                                    if 'A' not in ph_w:
                            ...
                            if ('A' in ph_w and 'B' not in ph_w and 'C' not in ph_w):
                                ...
                                DV = Pabc[0][0] * Pabc[1][1] * Pabc[2][2] - Pabc[0][0] * Pabc[1][2] * Pabc[2][1] - Pabc[0][1] * Pabc[1][0] * Pabc[2][
                                    2
                                ] + Pabc[0][1] * Pabc[2][0] * Pabc[1][2] + Pabc[1][0] * Pabc[0][2] * Pabc[2][1] - Pabc[0][2] * Pabc[1][1] * Pabc[2][0]
