...
#        inputfile = open(os.path.join(dew_models_dir,modelfile),'r')
            ...
            if nentries[0] == '$CMP,' and nentries[18]=='513,':
                ...
                while True:
                    ...
                    if row_node1[0] == '$CMP,' and row_node1[1] == node_path:
                        if row_node1[18]=='16,':
                            voltage_node = float(PTXFRM_DSECKV[int(row_node1[6][:-1])])*1.732*1000
                            break
                        elif row_node1[18]=='1032,':
                            voltage_node = float(PTSUB_DPHABKV[int(row_node1[6][:-1])])*1000 #multiplied by 1000 to match with ditto, ditto dividing by 1000
                            break
                        else:
                    ...
                    if obj_class == 'regulator':
                        ...
                        for kk in ph:
                            ...
                            while True:
                                ...
                                if row_reg[:13] == '$CMPSERIALNUM':
                                    ...
                                    try:
                                        ...
                                        api_regulator.connected_transformer = row_reg1[1][1:-2]+'_'+kk
                                    except AttributeError:
                                        pass                         
                            ...
                            if entries[0] == '$CMP,' and entries[5]=='22,':
                                ...
                                try:
                                    iptr = entries[6][:-1]                        
                                    api_regulator.highstep=int(PTXFRM_SNUMSTEPS[PTXFRM_IPTROW.index(float(iptr))])
                                    api_regulator.lowstep=int(PTXFRM_SNUMSTEPS[PTXFRM_IPTROW.index(float(iptr))])
        ...
        #
                            ...
                            tf_rcfg=APIXFRMCONIDX_STNAM[int(PTXFRM_IXFRMCON[PTXFRM_IPTROW.index(iptr_rwdg)])]
                            ...
                            while True:
                                ...
                                if row_pt2[:9] == '$CMPADDPT' and row_pt1[1] == '48,':
                                    try:
                                        ...
                                        pass
                                    try:
                                        api_regulator.ct_prim = float(PTINST_DSECDRATA[PTINST_IPTROW.index(float(row_pt1[2][:-1]))])*float(row_pt1[7][:-1])
                                    except AttributeError:
                                        pass
                ...
                if entries[0] == '$CMP,' and entries[18] == '16,':
                    ...
                    for nr in range(nreg):
                        ...
                        if obj_class == 'transformer':
                            ...
                            tf_cfg=APIXFRMCONIDX_STNAM[int(PTXFRM_IXFRMCON[PTXFRM_IPTROW.index(iptr_wdg)])]
                            ...
                            zang = float(PTXFRM_DVSAT0PC[int(PTXFRM_IPTROW.index(iptr_wdg))])
        ...
        #                        resistance = 0.001
            ...
            # TO-DO FOR 3-WINDING TRANSFORMER
                                if w == 0:
                                    try:
                                        if len(ph1) == 1:
                                            windings[w].nominal_voltage = float(PTXFRM_DPRIKV[int(PTXFRM_IPTROW.index(iptr_wdg))])*10**3/vmut
                                        else:
                                             windings[w].nominal_voltage = float(PTXFRM_DPRIKV[int(PTXFRM_IPTROW.index(iptr_wdg))])*10**3*pow(3,0.5)/vmut
                                        windings[w].rated_power = float(PTXFRM_DNOMKVA[int(PTXFRM_IPTROW.index(iptr_wdg))])*10**3/nreg
                                ...
                                if w == 1:
                                    try:
                                        if len(ph1) == 1:
                                            windings[w].nominal_voltage = float(PTXFRM_DSECKV[int(PTXFRM_IPTROW.index(iptr_wdg))])*10**3/vmut
                                        else:
                                            windings[w].nominal_voltage = float(PTXFRM_DSECKV[int(PTXFRM_IPTROW.index(iptr_wdg))])*10**3*pow(3,0.5)/vmut
                                        windings[w].rated_power = float(PTXFRM_DNOMKVA[int(PTXFRM_IPTROW.index(iptr_wdg))])*10**3/nreg
                                        if num_windings == 3:
             ...
             # Transformers Start
                        ...
                        while True:
                            ...
                            if row_tf[:13] == '$CMP_NEWDATA1':
                                break
                        try:
                            api_transformer.emergency_power=float(PTXFRM_DFAVLTMRATKVA[int(PTXFRM_IPTROW.index(iptr_wdg))])*10**3 #DiTTo in volt ampere
                        except:
                            pass
        
                        try:
                            api_transformer.loadloss=(float(PTXFRM_DWINDLOSSW[int(PTXFRM_IPTROW.index(iptr_wdg))])+float(PTXFRM_DCORELOSSW[int(PTXFRM_IPTROW.index(iptr_wdg))]))/1000.0 #DiTTo in volt ampere               
                        except:
                            pass
        
        
                        try:
                            api_transformer.normhkva=float(PTXFRM_DPRIKV[int(PTXFRM_IPTROW.index(iptr_wdg))])*10**3 #DiTTo in volt ampere
                        except:
                            pass    
        
                        try:
                            api_transformer.noload_loss=(float(PTXFRM_DCORELOSSW[int(PTXFRM_IPTROW.index(iptr_wdg))]))/1000.0 #DiTTo in volt ampere               
                        except:
                            pass
        ...
        #                    api_transformer.reactances.append(float(reactance*0.5))   #XHT
                            ...
                            if PTXFRM_QCOMPEXISTS[int(PTXFRM_IPTROW.index(iptr_wdg))] == 205.0:
                                zmag1 = float(PTXFRM_DISAT1A[int(PTXFRM_IPTROW.index(iptr_wdg))])
                                zang1 = float(PTXFRM_DVSAT1PC[int(PTXFRM_IPTROW.index(iptr_wdg))])
                                zmag2 = float(PTXFRM_DISAT2A[int(PTXFRM_IPTROW.index(iptr_wdg))])
                                zang2 = float(PTXFRM_DVSAT2PC[int(PTXFRM_IPTROW.index(iptr_wdg))])                        
                                reactance1 =  zmag1*math.sin(math.radians(zang1))
                                reactance2 =  zmag2*math.sin(math.radians(zang2))
                                api_transformer.reactances=[]
                                api_transformer.reactances.append(float(reactance1+reactance2))   #XHL
                            ...
                            while True:
                                ...
                                if row_frm1_ln[0] == '$CMP,' and row_frm1_ln[1] == frm_ln:
                                    ...
                                    if row_frm1_ln[18] == '513,':
                                        try:
                                            api_transformer.from_element = row_frm3_ln[1][1:-2]
                                        except AttributeError:
                                            pass
                                        break
                                    else:
        ...
        #
                        if entries[0] == '$CMP,' and entries[18]=='16,':
                            ...
                            while True:
                                ...
                                row_frm_ln = row_frm_ln.strip()
                                row_frm1_ln = row_frm_ln.split()
                                if row_frm1_ln[0] == '$CMP,' and row_frm1_ln[18] == '513,' and row_frm1_ln[32] == frm_ln:
                                    row_frm2_ln = all_rows[iter_frm_ln+1]
                                    row_frm2_ln = row_frm2_ln.strip()
                                    row_frm3_ln = row_frm2_ln.split()
                                    try:
                                        ...
                                        pass
                                    break
                                elif row_frm1_ln[0] == '$CMP,' and row_frm1_ln[18] == '513,' and row_frm1_ln[34] == frm_ln:
                                    row_frm2_ln = all_rows[iter_frm_ln+1]
                                    row_frm2_ln = row_frm2_ln.strip()
                                    row_frm3_ln = row_frm2_ln.split()
                                    try:
        ...
        # Transformers End  
            ...
            # Phase Capacitor
                ...
                if entries[0] == '$CMP,' and (entries[18] == '32,' or entries[18] == '40,'):
                    ...
                    if PTCAP_SCON[PTCAP_IPTROW.index(iptr_cap)] == 1.0 or PTCAP_SCON[PTCAP_IPTROW.index(iptr_cap)] == 3.0:
                        api_capacitor.connection_type='Y'
                        api_capacitor.nominal_voltage = float((APILEVIDX_DVARLEV[int(PTCAP_IVLEV[PTCAP_IPTROW.index(iptr_cap)])]))*10**3
                    elif PTCAP_SCON[PTCAP_IPTROW.index(iptr_cap)] == 2.0:
                        api_capacitor.connection_type='D'
                        api_capacitor.nominal_voltage = float((APILEVIDX_DVARLEV[int(PTCAP_IVLEV[PTCAP_IPTROW.index(iptr_cap)])]))*10**3
                    else:
                        api_capacitor.nominal_voltage = float((APILEVIDX_DVARLEV[int(PTCAP_IVLEV[PTCAP_IPTROW.index(iptr_cap)])])/1.732)*10**3
                        #single phase
                    
                    api_capacitor.low=float(APIRANIDX_DLOWVAL[int(PTCAP_IVRAN[PTCAP_IPTROW.index(iptr_cap)])])*1000  #cross check
                    api_capacitor.high=float(APIRANIDX_DUPVAL[int(PTCAP_IVRAN[PTCAP_IPTROW.index(iptr_cap)])])*1000  # cross check
                    ...
                    for p,p_c in enumerate(ph_c):
                        ...
                        if float(PTCAP_SCON[PTCAP_IPTROW.index(iptr_cap)]) in (1.0, 2.0, 3.0):
                            phase_capacitors[p].var=(float(PTCAP_DRATKVAR[PTCAP_IPTROW.index(iptr_cap)])*10**3)/3.0
                        else:
                            phase_capacitors[p].var=(float(PTCAP_DRATKVAR[PTCAP_IPTROW.index(iptr_cap)])*10**3)
                        phase_capacitors[p].sections=int(PTCAP_SNUMPOSRACK[PTCAP_IPTROW.index(iptr_cap)])
        ...
        # Capacitor End
        # Wire Data     
                if entries[0] == '$CMP,' and (entries[18]=='1,' or entries[18]=='65,' or  entries[18]=='129,' or entries[18]=='258,' or entries[18]=='2,' or entries[18]=='6,'):
...
#                        print(row_seq)
                    ...
                    for pw in range(num_ph+1):
                        ...
                        if pw >= num_ph:
                            ...
                            if int(entries[5][:-1]) == 8 or int(entries[5][:-1]) == 100:
                                ...
                                wires[pw].ampacity = float(PTSWT_DCURTRATA[int(entries[6][:-1])])
                                wires[pw].emergency_ampacity = float(PTSWT_DCURTRATA[int(entries[6][:-1])])*1.5
                            else:
                                wires[pw].is_fuse = 0
                            if int(entries[5][:-1]) in (4, 5, 120, 141, 216, 221, 222, 226, 238, 246, 6, 7, 53, 58, 76):
                                wires[pw].nameclass = PTSWT_STDESC[int(entries[6][:-1])]
                                wires[pw].is_switch = 1
                                wires[pw].resistance = 0.001
                                wires[pw].ampacity = float(PTSWT_DCURTRATA[int(entries[6][:-1])])
                                wires[pw].emergency_ampacity = float(PTSWT_DCURTRATA[int(entries[6][:-1])])
                            else:
                        ...
                        while True:
                            ...
                            if row_wr[:7] == '$CMPCHN':
                                ...
                                if int(entries[5][:-1]) == 35 or int(entries[5][:-1]) == 36 or int(entries[5][:-1]) == 37 or int(entries[5][:-1]) == 38:
            #                        if PTLINESPC_SOVERHEAD[int(row_wr1[4][:-1])] == 0.0:
                                    if pw >= num_ph:
                                        if int(row_wr1[6][:-1]) == -1:
                                            ...
                                            wires[pw].gmr = None
                                            wires[pw].ampacity = None #switches ampacity update it
                                            wires[pw].ampacity_emergency = None
                                            wires[pw].resistance = None #switches resistance update it
                                        else:
                                            wires[pw].nameclass=PTLINECOND_STDESC[int(row_wr1[6][:-1])]
                                            wires[pw].diameter=float(PTLINECOND_DRADCONDSUL[int(row_wr1[6][:-1])])*2
                                            wires[pw].gmr=float(PTLINECOND_DGMRSUL[int(row_wr1[6][:-1])])
                                            wires[pw].ampacity=float(PTLINECOND_DRATAMBTEMP0A[int(row_wr1[6][:-1])])
                                            wires[pw].ampacity_emergency=float(PTLINECOND_DRATAMBTEMP1A[int(row_wr1[6][:-1])])
                                            wires[pw].resistance=float(PTLINECOND_DROHMPRLUL[int(row_wr1[6][:-1])])*float(row_wr1[13][:-1])
                                    else:
            ...
            #                                print(float(PTLINECOND_DGMRSUL[int(row_wr1[5][:-1])]))
                                    ...
                                    if pw >= num_ph:
                                        if int(row_wr1[6][:-1]) == -1:
                                            ...
                                            wires[pw].nameclass=PTCABCOND_STDESC[int(row_wr1[6][:-1])]
                                            wires[pw].diameter=float(PTCABCOND_DRADCONDSUL[int(row_wr1[6][:-1])])*2
                                            wires[pw].gmr=float(PTCABCOND_DGMRSUL[int(row_wr1[6][:-1])])
                                            wires[pw].resistance=float(PTCABCOND_DROHMPRLUL[int(row_wr1[6][:-1])])*float(row_wr1[13][:-1])                                   
                                            if PTLINESPC_SOVERHEAD[int(row_wr1[4][:-1])] == 0.0:
                                                wires[pw].ampacity=float(PTCABCOND_DRATA0[int(row_wr1[6][:-1])])
                                                wires[pw].ampacity_emergency=float(PTCABCOND_DRATA0[int(row_wr1[5][:-1])])  #not provided
                                            if PTLINESPC_SOVERHEAD[int(row_wr1[4][:-1])] == 1.0:
                                                wires[pw].ampacity=float(PTCABCOND_DRATA1[int(row_wr1[6][:-1])])
                                                wires[pw].ampacity_emergency=float(PTCABCOND_DRATA1[int(row_wr1[5][:-1])])  #not provided
                                            if PTLINESPC_SOVERHEAD[int(row_wr1[4][:-1])] == 2.0:
                                                wires[pw].ampacity=float(PTCABCOND_DRATA2[int(row_wr1[6][:-1])])
                                                wires[pw].ampacity_emergency=float(PTCABCOND_DRATA2[int(row_wr1[5][:-1])])  #not provided
                                    else: 
                                        wires[pw].nameclass=PTCABCOND_STDESC[int(row_wr1[5][:-1])]
                                        wires[pw].diameter=float(PTCABCOND_DRADCONDSUL[int(row_wr1[5][:-1])])*2
                                        wires[pw].gmr=float(PTCABCOND_DGMRSUL[int(row_wr1[5][:-1])])
                                        wires[pw].resistance=float(PTCABCOND_DROHMPRLUL[int(row_wr1[5][:-1])])*float(row_wr1[13][:-1])  
                                        if PTLINESPC_SOVERHEAD[int(row_wr1[4][:-1])] == 0.0:
                                            wires[pw].ampacity=float(PTCABCOND_DRATA0[int(row_wr1[5][:-1])])
                                            wires[pw].ampacity_emergency=float(PTCABCOND_DRATA0[int(row_wr1[5][:-1])])  #not provided
                                        if PTLINESPC_SOVERHEAD[int(row_wr1[4][:-1])] == 1.0:
                                            wires[pw].ampacity=float(PTCABCOND_DRATA1[int(row_wr1[5][:-1])])
                                            wires[pw].ampacity_emergency=float(PTCABCOND_DRATA1[int(row_wr1[5][:-1])])   #not provided
                                        if PTLINESPC_SOVERHEAD[int(row_wr1[4][:-1])] == 2.0:
                                            wires[pw].ampacity=float(PTCABCOND_DRATA2[int(row_wr1[5][:-1])])
                                            wires[pw].ampacity_emergency=float(PTCABCOND_DRATA2[int(row_wr1[5][:-1])])  #not provided
                                        
                                if PTLINESPC_TMUTSPC[int(row_wr1[4][:-1])] == 0.0 or PTLINESPC_TMUTSPC[int(row_wr1[4][:-1])] == 1.0:
                                    if pw >= num_ph:
                                        wires[pw].X =  float(PTLINESPC_DXNEU[int(row_wr1[4][:-1])])*0.3048-float(PTLINESPC_DXPH1ORR1[int(row_wr1[4][:-1])])*0.3048
                                        wires[pw].Y =  float(PTLINESPC_DYNEU[int(row_wr1[4][:-1])])*0.3048-float(PTLINESPC_DYPH1ORX1[int(row_wr1[4][:-1])])*0.3048
                                    else:
                                        if pw == 0:
                                            wires[pw].X =  float(PTLINESPC_DXPH1ORR1[int(row_wr1[4][:-1])])*0.3048    # feet to meter converted
                                            wires[pw].Y =  float(PTLINESPC_DYPH1ORX1[int(row_wr1[4][:-1])])*0.3048    # feet to meter converted
                                        if pw == 1:
                                            wires[pw].X =  float(PTLINESPC_DXPH2ORR0[int(row_wr1[4][:-1])])*0.3048-float(PTLINESPC_DXPH1ORR1[int(row_wr1[4][:-1])])*0.3048
                                            wires[pw].Y =  float(PTLINESPC_DYPH2ORX0[int(row_wr1[4][:-1])])*0.3048-float(PTLINESPC_DYPH1ORX1[int(row_wr1[4][:-1])])*0.3048
                                        if pw == 2:
                                            wires[pw].X =  float(PTLINESPC_DXPH3ORY0[int(row_wr1[4][:-1])])*0.3048-float(PTLINESPC_DXPH1ORR1[int(row_wr1[4][:-1])])*0.3048
                                            wires[pw].Y =  float(PTLINESPC_DYPH3ORY1[int(row_wr1[4][:-1])])*0.3048-float(PTLINESPC_DYPH1ORX1[int(row_wr1[4][:-1])])*0.3048
                                            
                            if row_wr[:8] == '$CMPOPER':
                                row_wr=row_wr.strip()
                                row_wr1=row_wr.split()
                                if (int(entries[5][:-1]) == 8 or int(entries[5][:-1]) == 100) and (int(row_wr1[5]) == 1):
                                    wires[pw].is_open = 1  # blown
                                if (int(entries[5][:-1]) == 8 or int(entries[5][:-1]) == 100) and (int(row_wr1[5]) == 0):
                                    wires[pw].is_open = 0  # good
                                if (int(entries[5][:-1]) in (4, 5, 120, 141, 216, 221, 222, 226, 238, 246, 6, 7, 53, 58, 76)) and (row_wr1[5] == str(1)):
                                    wires[pw].is_open = 1  # open 
                                if (int(entries[5][:-1]) in (4, 5, 120, 141, 216, 221, 222, 226, 238, 246, 6, 7, 53, 58, 76)) and (row_wr1[5] == str(0)):
                                    wires[pw].is_open = 0  # close                     
                            if row_wr[:13] == '$CMP_NEWDATA1':
                                break
                    api_line=Line(model)
                    api_line.wires = wires
                    if (entries[5] == '35,' or entries[5] == '36,' or entries[5] == '37,' or entries[5] == '38,' or entries[5] == '39,' or entries[5] == '40,' or entries[5] == '41,' or entries[5] == '41,'):
                        api_line.line_type = 'overhead'
                    if (entries[5] == '43,' or entries[5] == '44,' or entries[5] == '45,' or entries[5] == '46,'):
        ...
        # cable data impedance matrix calculation
                    ...
                    Freq_Coeff = 0.00158836*60+0.00202237*60j;  #60Hz Freq
                    Freq_addit = math.log(100/60)/2.0 + 7.6786;  # earth resistivity 100 and freq 60
                    ...
                    while True:
                        ...
                        if row_ug[:7] == '$CMPCHN':
                            ...
                            if int(entries[5][:-1]) == 43 or int(entries[5][:-1]) == 44 or int(entries[5][:-1]) == 45 or int(entries[5][:-1]) == 46:
                                if (PTLINESPC_TMUTSPC[int(row_ug1[4][:-1])] == 0.0 or PTLINESPC_TMUTSPC[int(row_ug1[4][:-1])] == 1.0):
                                    cond_dia1 = float(PTCABCOND_DRADCONDSUL[int(row_ug1[5][:-1])])*2
                                    cond_dia2 = float(PTCABCOND_DRADCONDSUL[int(row_ug1[5][:-1])])*2
                                    cond_dia3 = float(PTCABCOND_DRADCONDSUL[int(row_ug1[5][:-1])])*2
                                    cond_res1 = float(PTCABCOND_DROHMPRLUL[int(row_ug1[5][:-1])])*5.28 #LUL TO MILE
                                    cond_res2 = float(PTCABCOND_DROHMPRLUL[int(row_ug1[5][:-1])])*5.28 #LUL TO MILE
                                    cond_res3 = float(PTCABCOND_DROHMPRLUL[int(row_ug1[5][:-1])])*5.28 #LUL TO MILE
                                    ...
                                    permA = float(PTINSULIDX_DRELATIVEPERMIT[int(PTCABCOND_IINSUL[int(row_ug1[5][:-1])])])
                                    if row_ug1[6] == '-1,':
                                        cond_dia7 = 0.0
                                        cond_res7 = 0.0 #LUL TO MILE
                                    else:
                                        cond_dia7 = float(PTCABCOND_DRADCONDSUL[int(row_ug1[6][:-1])])/12
                                        cond_res7 = float(PTCABCOND_DROHMPRLUL[int(row_ug1[6][:-1])])*5.28 #LUL TO MILE
                                    if PTCABCOND_TCONCENTNEU[int(row_ug1[5][:-1])] == 1.0:  #CONCENTRIC NEUTRAL
                                        out_dia1 = float(PTCABCOND_DDIANEUSTRNDSUL[int(row_ug1[5][:-1])])
                                        out_dia2 = float(PTCABCOND_DDIANEUSTRNDSUL[int(row_ug1[5][:-1])])
                                        out_dia3 = float(PTCABCOND_DDIANEUSTRNDSUL[int(row_ug1[5][:-1])])
                                        GMR1 = float(PTCABCOND_DGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR2 = float(PTCABCOND_DGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR3 = float(PTCABCOND_DGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR4C = float(PTCABCOND_DNEUSTRNDGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR5C = float(PTCABCOND_DNEUSTRNDGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR6C = float(PTCABCOND_DNEUSTRNDGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR4S = 0
                                        GMR5S = 0
                                        GMR6S = 0
                                        cond_dia4 = float(PTCABCOND_DRADSTRNDSUL[int(row_ug1[5][:-1])])*2
                                        cond_dia5 = float(PTCABCOND_DRADSTRNDSUL[int(row_ug1[5][:-1])])*2
                                        cond_dia6 = float(PTCABCOND_DRADSTRNDSUL[int(row_ug1[5][:-1])])*2
                                        cond_res4 = float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])])*5.28
                                        cond_res5 = float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])])*5.28
                                        cond_res6 = float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])])*5.28
                                        cond_res4S = 0
                                        cond_res5S = 0
                                        cond_res6S = 0
                                        sheild_thick1 = 0;
                                        ...
                                        sheild_dia3 = 0;
                                        sheild_dia4 = 0;
                                        nue_strands4 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                        nue_strands5 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                        nue_strands6 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                        ...
                                        GMRcn4 = pow(GMR4C*nue_strands4*pow(R14,nue_strands4-1),1/nue_strands4)
                                        GMRcn5 = pow(GMR5C*nue_strands5*pow(R25,nue_strands5-1),1/nue_strands5)
                                        GMRcn6 = pow(GMR6C*nue_strands6*pow(R36,nue_strands6-1),1/nue_strands6)
                                        if row_ug1[6] == '-1,':
                                            GMR7 = 0.0
                                            D17 = 0
                                            D27 = 0
                                            ...
                                            GMR7 = float(PTCABCOND_DGMRSUL[int(row_ug1[6][:-1])])/12
                                            D17 = abs(XN-XA)
                                            D27 = abs(XN-XB)
                                            D37 = abs(XN-XC)
                                    elif PTCABCOND_TCONCENTNEU[int(row_ug1[5][:-1])] == 0.0:   #TAPE SHEILD
                                        out_dia1 = float(PTCABCOND_DCONDJACKETSUL[int(row_ug1[5][:-1])])
                                        out_dia2 = float(PTCABCOND_DCONDJACKETSUL[int(row_ug1[5][:-1])])
                                        out_dia3 = float(PTCABCOND_DCONDJACKETSUL[int(row_ug1[5][:-1])])
                                        GMR1 = float(PTCABCOND_DGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR2 = float(PTCABCOND_DGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR3 = float(PTCABCOND_DGMRSUL[int(row_ug1[5][:-1])])/12
                                        GMR4C = 0
                                        GMR5C = 0
                                        GMR6C = 0
                                        Tape_Thick = float(PTCABCOND_DDIANEUSTRNDSUL[int(row_ug1[5][:-1])])
                                        GMR4S = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])+Tape_Thick)/24.0 #recheck this
                                        GMR5S = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])+Tape_Thick)/24.0 #recheck this
                                        GMR6S = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])+Tape_Thick)/24.0 #recheck this
                                        cond_dia4 = 0
                                        cond_dia5 = 0
                                        cond_dia6 = 0
                                        cond_res4 = 0
                                        cond_res5 = 0
                                        cond_res6 = 0 
                                        cond_res4S = ((7.9385e8)*0.3048*float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])]))/(float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])*Tape_Thick*1000)
                                        cond_res5S = ((7.9385e8)*0.3048*float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])]))/(float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])*Tape_Thick*1000)
                                        cond_res6S = ((7.9385e8)*0.3048*float(PTCABCOND_DNEUSTRNDROHM[int(row_ug1[5][:-1])]))/(float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])*Tape_Thick*1000)           
                                        sheild_thick1 = Tape_Thick;
                                        sheild_thick2 = Tape_Thick;
                                        sheild_thick3 = Tape_Thick;
                                        sheild_dia1 = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])+2*Tape_Thick); #recheck this
                                        sheild_dia2 = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])+2*Tape_Thick); #recheck this
                                        sheild_dia3 = (float(PTCABCOND_DINSULSUL[int(row_ug1[5][:-1])])+2*Tape_Thick); #recheck this
                                        nue_strands4 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                        nue_strands5 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                        nue_strands6 = float(PTCABCOND_SNUMNEUSTRND[int(row_ug1[5][:-1])])
                                        ...
                                        if row_ug1[6] == '-1,':
                                            ...
                                            GMR7 = float(PTCABCOND_DGMRSUL[int(row_ug1[6][:-1])])/12
                                            sheild_thick4 = float(PTCABCOND_DDIANEUSTRNDSUL[int(row_ug1[6][:-1])]);
                                            sheild_dia4 = (float(PTCABCOND_DINSULSUL[int(row_ug1[6][:-1])])+float(PTCABCOND_DDIANEUSTRNDSUL[int(row_ug1[6][:-1])])); #recheck this
                                            cond_res7 = float(PTCABCOND_DROHMPRLUL[int(row_ug1[6][:-1])])*5.28
                                            D17 = abs(XN-XA)
                                            D27 = abs(XN-XB)
                                            D37 = abs(XN-XC)
                                    #1 = phase conductor #1
                                    ...
                                    Rc[4] = Rcn5
                                    Rc[5] = Rcn6
                                    dis = ([[0.0,D12,D13,D14,D15,D16,D17],
                                                ...
                                                if i_cn == j_cn:
                                                    if (i_cn > 2 and i_cn != 6):
                                                        if GMRc[i_cn] == 0.0:
                                                            ...
                                                            Z[i_cn][j_cn] = complex(Freq_Coeff.real + Rc[i_cn],Freq_Coeff.imag * (math.log(1.0 / GMRc[i_cn]) + Freq_addit))
                                                    else:
                                                        if GMR[i_cn] == 0.0:
                                                            Z[i_cn][j_cn] = complex(0.0,0.0)
                                                        else:
                                                            Z[i_cn][j_cn] = complex(Freq_Coeff.real + cond_res[i_cn], Freq_Coeff.imag * (math.log(1.0 / GMR[i_cn]) + Freq_addit))
                                                else:
                                                    if dis[i_cn][j_cn] == 0.0:
                                                        Z[i_cn][j_cn] = complex(0.0,0.0)
                                                    else:
                                                        Z[i_cn][j_cn] = complex(Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / dis[i_cn][j_cn]) + Freq_addit))
                                        zijcn =  np.matrix([[Z[0][0], Z[0][1], Z[0][2]], [Z[1][0], Z[1][1], Z[1][2]], [Z[2][0], Z[2][1], Z[2][2]]])
                                        zincn =  np.matrix([[Z[0][3], Z[0][4], Z[0][5]], [Z[1][3], Z[1][4], Z[1][5]], [Z[2][3], Z[2][4], Z[2][5]]])
                                        znjcn =  np.matrix([[Z[3][0], Z[3][1], Z[3][2]], [Z[4][0], Z[4][1], Z[4][2]], [Z[5][0], Z[5][1], Z[5][2]]])
                                        znncn =  np.matrix([[Z[3][3], Z[3][4], Z[3][5]], [Z[4][3], Z[4][4], Z[4][5]], [Z[5][3], Z[5][4], Z[5][5]]])
                                        if 'A' not in ph_w:
                                    ...
                                    # capacitance calculation
                                        if 'A' in ph_w:
                                            if cond_dia1 == 0 or R14 == 0 or nue_strands4 == 0:
                                                can = 0
                                            else:
                                                den_aph = math.log(R14/(cond_dia1 / 24.0)) - (1.0 /nue_strands4) * math.log((nue_strands4)*cond_dia4 / 24.0 / R14)
                                                if den_aph == 0:
                                                    can = 0
                                                else:
                                                    can = (2.0 * math.pi * 0.01420 * permA) / den_aph
                                        else:
                                            can = 0
                                        if 'B' in ph_w:
                                            if cond_dia2 == 0 or R25 == 0 or nue_strands5 == 0:
                                                cbn = 0
                                            else:
                                                den_bph = math.log(R25/(cond_dia2 / 24.0)) - (1.0 /nue_strands5) * math.log((nue_strands5)*cond_dia5 / 24.0 / R25)
                                                if den_bph == 0:
                                                    cbn = 0
                                                else:
                                                    cbn = (2.0 * math.pi * 0.01420 * permA) / den_bph
                                        else:
                                            cbn = 0
                                        if 'C' in ph_w:
                                            if cond_dia3 == 0 or R36 == 0 or nue_strands6 == 0:
                                                ccn = 0
                                            else:
                                                den_cph = math.log(R36/(cond_dia3 / 24.0)) - (1.0 /nue_strands6) * math.log((nue_strands6)*cond_dia6 / 24.0 / R36)
                                                if den_cph == 0:
                                                    ccn = 0
                                                else:
                                                    ccn = (2.0 * math.pi * 0.01420 * permA) / den_cph
                                        else:
                                            ccn = 0
                                                ...
                                                if i_cn == j_cn:
                                                    if (i_cn > 2 and i_cn != 6):
                                                        if GMRS[i_cn] == 0.0:
                                                            ...
                                                            Z[i_cn][j_cn] = complex(Freq_Coeff.real+shld_res[i_cn], Freq_Coeff.imag*(math.log(1.0/GMRS[i_cn]) + Freq_addit))
                                                    elif i_cn == 6:
                                                        if 'A' in ph_w:
                                                            Z[i_cn][j_cn] = complex(Freq_Coeff.real + cond_res4S, Freq_Coeff.imag * (math.log(1.0 / GMR4S) + Freq_addit))
                                                        elif 'B' in ph_w:
                                                            Z[i_cn][j_cn] = complex(Freq_Coeff.real + cond_res5S, Freq_Coeff.imag * (math.log(1.0 / GMR5S) + Freq_addit))
                                                        elif 'C' in ph_w:
                                                            Z[i_cn][j_cn] = complex(Freq_Coeff.real + cond_res6S, Freq_Coeff.imag * (math.log(1.0 / GMR6S) + Freq_addit))
                                                    else:
                                                        if GMRS == 0:
                                                            Z[i_cn][j_cn] = complex(0.0)
                                                        else:
                                                    ...
                                                    if ((i_cn == 0 and j_cn == 3) or (i_cn == 1 and j_cn == 4) or (i_cn == 2 and j_cn == 5)):
                                                        if GMRS == 0:
                                                            Z[i_cn][j_cn] = complex(0.0)
                                                        else:
                                                            Z[i_cn][j_cn] = complex(Freq_Coeff.real, Freq_Coeff.imag*(math.log(1.0/GMRS[i_cn]) + Freq_addit))
                                                    else:
                                                        if dis[i_cn][j_cn] == 0:
                                                            Z[i_cn][j_cn] = complex(0.0)
                                                        else:
                                                            Z[i_cn][j_cn] = complex(Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / dis[i_cn][j_cn]) + Freq_addit))
                                        zijts = np.matrix([[Z[0][0], Z[0][1], Z[0][2]], [Z[1][0], Z[1][1], Z[1][2]], [Z[2][0], Z[2][1], Z[2][2]]])
                                        zints =  np.matrix([[Z[0][3], Z[0][4], Z[0][5], Z[0][6]], [Z[1][3], Z[1][4], Z[1][5], Z[1][6]], [Z[2][3], Z[2][4], Z[2][5], Z[2][6]]])
                                        znjts =  np.matrix([[Z[3][0], Z[3][1], Z[3][2]], [Z[4][0], Z[4][1], Z[4][2]], [Z[5][0], Z[5][1], Z[5][2]], [Z[6][0], Z[6][1], Z[6][2]]])
                                        znnts =  np.matrix([[Z[3][3], Z[3][4], Z[3][5], Z[3][6]], [Z[4][3], Z[4][4], Z[4][5], Z[4][6]], [Z[5][3], Z[5][4], Z[5][5], Z[5][6]], [Z[6][3], Z[6][4], Z[6][5], Z[6][6]]])
                                        if 'A' not in ph_w:
                            ...
                            elif int(entries[5][:-1]) == 35 or int(entries[5][:-1]) == 36 or int(entries[5][:-1]) == 37 or int(entries[5][:-1]) == 38:
                                DAB = 0.0
                                DBC = 0.0
                                DAC = 0.0
                                DAN = 0.0
                                ...
                                DBE = 0.0
                                DCE = 0.0
                                GMRA_OH = float(PTLINECOND_DGMRSUL[int(row_ug1[5][:-1])])/12.0
                                GMRB_OH = float(PTLINECOND_DGMRSUL[int(row_ug1[5][:-1])])/12.0
                                GMRC_OH = float(PTLINECOND_DGMRSUL[int(row_ug1[5][:-1])])/12.0
                                RESA_OH = float(PTLINECOND_DROHMPRLUL[int(row_ug1[5][:-1])])*5.28
                                RESB_OH = float(PTLINECOND_DROHMPRLUL[int(row_ug1[5][:-1])])*5.28
                                RESC_OH = float(PTLINECOND_DROHMPRLUL[int(row_ug1[5][:-1])])*5.28
                                DIAA = float(PTLINECOND_DRADCONDSUL[int(row_ug1[5][:-1])])*2
                                DIAB = float(PTLINECOND_DRADCONDSUL[int(row_ug1[5][:-1])])*2
                                DIAC = float(PTLINECOND_DRADCONDSUL[int(row_ug1[5][:-1])])*2
                                if PTLINESPC_TMUTSPC[int(row_ug1[4][:-1])] == 0.0 or PTLINESPC_TMUTSPC[int(row_ug1[4][:-1])] == 1.0:
                                ...
                                if int(row_ug1[6][:-1]) == -1:
                                    ...
                                    GMRN_OH = float(PTLINECOND_DGMRSUL[int(row_ug1[6][:-1])])/12.0
                                    RESN_OH = float(PTLINECOND_DROHMPRLUL[int(row_ug1[6][:-1])])*5.28
                                    DIAN = float(PTLINECOND_DRADCONDSUL[int(row_ug1[6][:-1])])*2    
                                if 'A' in ph_w:
                                    if (GMRA_OH > 0.0 and RESA_OH > 0.0):
                                        ZAA = complex(RESA_OH + Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / GMRA_OH) + Freq_addit))
                                    else:
                                        ZAA = 0.0
                                    if ('B' in ph_w and DAB > 0.0):
                                        ZAB = complex(Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / DAB) + Freq_addit))
                                    else:
                                        ZAB = 0.0
                                    if ('C' in ph_w and DAC > 0.0):
                                        ZAC = complex(Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / DAC) + Freq_addit))
                                    else:
                                        ZAC = 0.0
                                    if (int(row_ug1[6][:-1]) != -1 and DAN > 0.0):
                                        ZAN = complex(Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / DAN) + Freq_addit))
                                    else:
                                        ZAN = 0.0
                                    if ((DIAA > 0.0) and (DAE > 0.0)):
                                        ...
                                        if 'B' in ph_w:
                                            DABP = pow((pow(XB-XA,2)+pow(-YB-YA,2)),0.5)
                                            if DAB != 0:
                                            ...
                                            PAB = 0.0
                                        if 'C' in ph_w:
                                            DACP = pow((pow(XC-XA,2)+pow(-YC-YA,2)),0.5)
                                            if DAC != 0:
                                            ...
                                            PAC = 0.0
                                        if int(row_ug1[6][:-1]) != -1:
                                            DANP = pow((pow(XN-XA,2)+pow(-YN-YA,2)),0.5)
                                            if DAN != 0:
                                ...
                                if 'B' in ph_w:
                                    if (GMRB_OH > 0.0 and RESB_OH > 0.0):
                                        ZBB = complex(RESB_OH + Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / GMRB_OH) + Freq_addit))
                                    else:
                                        ZBB = 0.0
                                    if ('C' in ph_w and DBC > 0.0):
                                        ZBC = complex(Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / DBC) + Freq_addit))
                                    else:
                                        ZBC = 0.0
                                    if (int(row_ug1[6][:-1]) != -1 and DBN > 0.0):
                                        ZBN = complex(Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / DBN) + Freq_addit))
                                    else:
                                        ZBN = 0.0
                                    if ((DIAB > 0.0) and (DBE > 0.0)):
                                        ...
                                        if 'C' in ph_w:
                                            DBCP = pow((pow(XC-XB,2)+pow(-YC-YB,2)),0.5)
                                            if DBC != 0:
                                            ...
                                            PBC = 0.0
                                        if int(row_ug1[6][:-1]) != -1:
                                            DBNP = pow((pow(XN-XB,2)+pow(-YN-YB,2)),0.5)
                                            if DBN != 0:
                                ...
                                if 'C' in ph_w:
                                    if (GMRC_OH > 0.0 and RESC_OH > 0.0):
                                        ZCC = complex(RESC_OH + Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / GMRC_OH) + Freq_addit))
                                    else:
                                        ZCC = 0.0
                                    if (int(row_ug1[6][:-1]) != -1 and DCN > 0.0):
                                        ZCN = complex(Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / DCN) + Freq_addit))
                                    else:
                                        ZCN = 0.0
                                    if ((DIAC > 0.0) and (DCE > 0.0)):
                                        ...
                                        if int(row_ug1[6][:-1]) != -1:
                                            DCNP = pow((pow(XN-XC,2)+pow(-YN-YC,2)),0.5)
                                            if DCN != 0:
                                    ...
                                    ZCN = 0.0
                                ZNN_INV = complex(0,0)
                                if (int(row_ug1[6][:-1]) != -1 and GMRN_OH > 0.0 and RESN_OH > 0.0):
                                    ZNN = complex(RESN_OH + Freq_Coeff.real, Freq_Coeff.imag * (math.log(1.0 / GMRN_OH) + Freq_addit))
                                    ZNN_INV = 1/ZNN
                                ...
                                if ('A' in ph_w and 'B' not in ph_w and 'C' not in ph_w):
                                    ...
                                    DV = Pabc[0][0]*Pabc[1][1]*Pabc[2][2] - Pabc[0][0]*Pabc[1][2]*Pabc[2][1] - Pabc[0][1]*Pabc[1][0]*Pabc[2][2] + Pabc[0][1]*Pabc[2][0]*Pabc[1][2] + Pabc[1][0]*Pabc[0][2]*Pabc[2][1] - Pabc[0][2]*Pabc[1][1]*Pabc[2][0]
                                    Yabc[0][0] = (Pabc[1][1]*Pabc[2][2] - Pabc[1][2]*Pabc[2][1]) / DV * Cap_Freq
                                    Yabc[0][1] = (Pabc[0][2]*Pabc[2][1] - Pabc[0][1]*Pabc[2][2]) / DV * Cap_Freq
                                    Yabc[0][2] = (Pabc[0][1]*Pabc[1][2] - Pabc[0][2]*Pabc[1][1]) / DV * Cap_Freq
                                    Yabc[1][0] = (Pabc[2][0]*Pabc[1][2] - Pabc[1][0]*Pabc[2][2]) / DV * Cap_Freq
                                    Yabc[1][1] = (Pabc[0][0]*Pabc[2][2] - Pabc[0][2]*Pabc[2][0]) / DV * Cap_Freq
                                    Yabc[1][2] = (Pabc[1][0]*Pabc[0][2] - Pabc[0][0]*Pabc[1][2]) / DV * Cap_Freq
                                    Yabc[2][0] = (Pabc[1][0]*Pabc[2][1] - Pabc[1][1]*Pabc[2][0]) / DV * Cap_Freq
                                    Yabc[2][1] = (Pabc[0][1]*Pabc[2][0] - Pabc[0][0]*Pabc[2][1]) / DV * Cap_Freq
                                    Yabc[2][2] = (Pabc[0][0]*Pabc[1][1] - Pabc[0][1]*Pabc[1][0]) / DV * Cap_Freq
                            ...
                            if row_ld_n[:7] == '$CMPCHN':
                                ...
                                try:
                                    if row_ld1_n[17][:-1] == '1.000000':
                                        ...
                                        api_load.nominal_voltage = (node_volt_dict[load_node])/pow(3,0.5)
