		...
		for name,data in lines.iteritems():
			...
			if line_geometry_code is not None:
				try:
					...
					self.logger.warning('Could not get the geometry {line_geom} data of line {line_name}'.format(line_geom=line_geometry_code, line_name=name))
				...
				if this_line_geometry is not None:
					...
					if this_line_wireData_code is not None:
						try:
							...
							self.logger.warning('Could not get the wireData {wiredata} of lineGeometry {line_geom}'.format(wiredata=this_line_wireData_code, line_geom=this_line_geometry))
							pass
					else:
