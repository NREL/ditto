            ...
            for object_property, property_value in _object.items():
                ...
                if object_property != 'klass':
                    ...
                    if property_type == 'list':
                        ...
                        for element in property_value['value']:
                            ...
                            if element_type in ditto_klasses:
                                ...
                                #Ex: Winding
                                api_object_one_level_deep = eval(element_type)(self.model)
                                    ...
                                    if element_property != 'klass':
                                        ...
                                        if nested_element_type == 'list':
                                            ...
                                            for element_deep in element_value['value']:
                                                ...
                                                #Get the class
                                                element_deep_class = element_deep['klass']
                                                ...
                                                if element_deep_class in ditto_klasses:
                                                    ...
                                                    for nested_object_property, nested_object_property_value in element_deep.items():
                                                        ...
                                                        if nested_object_property != 'klass':
                                                            ...
                                                            if nested_object_property_value['klass'] == 'complex':

                                                                setattr(
                                                                    api_object_two_level_deep, nested_object_property,
                                                                    complex(
                                                                        nested_object_property_value['value'][0],
                                                                        nested_object_property_value['value'][1]
                                                                    )
                                                                )

                                                            #...or a standard type
                                                            elif nested_object_property_value['klass'] != 'NoneType':

                                                                setattr(
                                                                    api_object_two_level_deep, nested_object_property,
                                                                    eval(nested_object_property_value['klass'])(
                                                                        nested_object_property_value['value']
                                                                    )
                                                                )

                                                    #Add the DiTTo object to the list
                                                    list_second_level.append(api_object_two_level_deep)

                                                #if it is a complex number, cast and add to the list
                                                elif element_deep_class == 'complex':

                                                    list_second_level.append(complex(element_deep['value'][0], element_deep['value'][1]))

                                                #If it is a standard type, cast and add to the list
                                                elif element_deep_class != 'NoneType':

                                                    list_second_level.append(eval(element_deep_class)(element_deep['value']))

                                            #Set the object attribute with the list we just built
                                            setattr(api_object_one_level_deep, element_property, list_second_level)

                                        #Or, it could be a complex number
                                        elif nested_element_type == 'complex':

                                            setattr(
                                                api_object_one_level_deep, element_property,
                                                complex(element_value['value'][0], element_value['value'][1])
                                            )

                                        #Or, None or standard types
                                        elif nested_element_type != 'NoneType':

                                            setattr(api_object_one_level_deep, element_property, eval(nested_element_type)(element_value['value']))
                            ...
                            #If it is a list, this should be a matrix stored as a list of lists
                                ...
                                for element_deep in element['value']:
                                    ...
                                    if element_deep['klass'] == 'complex':
                                        ...
                                        inner_list.append(complex(element_deep['value'][0], element_deep['value'][1]))

                                    #Or standard numbers (int or float)
                                    elif element_deep['klass'] != 'NoneType':

                                        inner_list.append(eval(element_deep['klass'])(element_deep['value']))
                            ...
                            #Or, it could be a complex number (handled in a list format)
                                ...
                                list_first_level.append(complex(element['value'][0], element['value'][1]))

                            #Otherwise, it is either None or a standard type
                            elif element_type != 'NoneType':

                                list_first_level.append(eval(element_type)(element['value']))
                    ...
                    #the representation used is a list [nb.real,nb.imag]
                        ...
                        setattr(api_object, object_property, complex(property_value['value'][0], property_value['value'][1]))

                    #If it is None, there is nothing to do
                    #Otherwise, it is a standard type like int, float, str...
                    elif property_type != 'NoneType':

                        setattr(api_object, object_property, eval(property_type)(property_value['value']))
