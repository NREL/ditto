...
class Reader(AbstractReader):
    ...
    def create_device(self, model, node1, node2, Devices):
        ...
        for phase ,phase_index in zip(phases, phase_list):
            ...
            phase_device.is_open = bool(self.nxGraph[node1][node2]['isClosed'][phase_index])
            ...
            phase_device.fuse_limit = float(device_data['Max Asymmetrical Fault'].iloc[0])
    ...
    def create_switch(self, model, node1, node2):
        ...
        for phase ,phase_index in zip(phases, phase_list):
            ...
            phase_sw.is_open = True if self.nxGraph[node1][node2]['state'] == 'O' else False
    ...
    def create_line(self, model, node1, node2, OHcables, UGcables):
        ...
        for phase ,phase_index in zip(phases, phase_list):
            ...
            if self.nxGraph[node1][node2]['type'] == 'overhead':
                ...
                if len(cond_data):
                    phase_wire.gmr = self.convert_to_meters(float(cond_data['Geometric Mean Radius'].iloc[0]), 'ft')
                    phase_wire.diameter = self.convert_to_meters(float(cond_data['Diameter'].iloc[0]), 'in')
                    phase_wire.ampacity = float(cond_data['Carrying Capacity'].iloc[0])
                    phase_wire.resistance = self.convert_from_meters(float(cond_data['Resistance @ 50'].iloc[0]), 'mi')
            if self.nxGraph[node1][node2]['type'] == 'underground':
                ...
                if len(cond_data):
                    ...
                    phase_wire.concentric_neutral_diameter = self.convert_to_meters(
                        float(cond_data['OD of Cable Including Neutral In Fee'].iloc[0]), 'ft')
                    phase_wire.ampacity = float(cond_data['Carrying Capacity In Amps'].iloc[0])
                    phase_wire.resistance = self.convert_from_meters(
                        float(cond_data['Phase Conductor Resistance Ohms/Mile'].iloc[0]), 'mi')
                    phase_wire.concentric_neutral_resistance = self.convert_from_meters(
                        float(cond_data['Concentric Neutral Resist Ohms/Mile'].iloc[0]), 'ft')
#
    ...
    def parse_loads(self, model):
        ...
        for node in self.nxGraph.nodes():
            if 'loads' in self.nxGraph.node[node] and len(list(self.nxGraph.edges([node]))):
    ...
    def parse_capacitors(self, model):
        for node in self.nxGraph.nodes():
            if 'capacitors' in self.nxGraph.node[node] and len(list(self.nxGraph.edges([node]))):
                node1, node2 = list(self.nxGraph.edges([node]))[0]
                for cap_name, cap_properties in self.nxGraph.node[node]['loads'].items():
                    if cap_properties['state'] != 'Disconnected':
                        ...
                        for phase, phase_index in zip(phases, phase_list):
                            ...
                            cap_phase.switch = 0 if cap_properties['state'] == 'Off' else 1
