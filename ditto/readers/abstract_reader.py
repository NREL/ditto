...
class AbstractReader(object):
    ...
    def kron_reduction(self, primitive_impedance_matrix, neutrals=None):
        ...
        if neutrals is None: #assume last row is neutral if nothing provided
            logger.warning("Warning - last row assumed to be used for Kron reduction")
    ...
    def get_phase_impedances(self,wire_list, distance_matrix = None, overhead=True, kron_reduce=None):
        ...
        if distance_matrix is None:
            ...
            for i in range(nphases):
                for j in range(nphases):
                    if overhead:
                        distance_matrix[i][j] = abs(i-j)*0.6 #Assume 60 cm apart one above another or side by side.
                    else:
                        distance_matrix[i][j] = abs(i-j)*0.3 # Assume 30 cm apart next to each other.
        ...
        for i in range(nphases):
            for j in range(nphases):
                distance_matrix_feet[i][j] = self.convert_from_meters(distance_matrix[i][j],'ft')
        ...
        if overhead:
            ...
            for i in range(nphases):
                if wire_list[i].gmr is None:
                    ...
                    logger.warning("Warning - using default gmr of "+str(gmr))
                else:
                    gmr_meters = wire_list[i].gmr
                if wire_list[i].resistance is None:
                    resistance_meters = 0.1194 / 1000 # ACSR Hawk (477kcmil)
                    logger.warning("Warning - using default resistance of "+str(resistance))
                else:
                    resistance_meters = wire_list[i].resistance
                gmrs.append(self.convert_from_meters(gmr_meters,'ft'))
                rs.append(self.convert_from_meters(resistance_meters,'ft',inverse=True))
            impedance_matrix_imperial = self.get_primitive_impedance_matrix(np.array(distance_matrix_feet),gmrs,rs)
            if kron_reduce:
                impedance_matrix_imperial = self.kron_reduce(impedance_matrix_imperial) # automatically assumes last element ei
            for i in range(len(impedance_matrix)):
                for j in range(len(impedance_matrix)):
                    impedance_matrix[i][j] = self.convert_to_meters(impedance_matrix_imperial[i][j],'mi',inverse=True) #works for complex
        else:
            ...
            for i in range(nphases):
                if wire_list[i].concentric_neutral_gmr is None: # WARNING: this is the GMR of the neutral strand, not the computed GMR of all neutrals
                    concentric_neutral_gmr_meters = 0.5 /1000.0* 0.7788 # half a mm gmr is the radius of the strand.
                    logger.warning('Warning - using default concentric_neutral_gmr of '+str(concentric_neutral_gmr_meters))
                else:
                ...
                if wire_list[i].concentric_neutral_nstrand is None:
                    ...
                    logger.warning('Warning - using default concentric_neutral_nstrand of '+str(concentric_neutral_nstrand))
                else:
                    concentric_neutral_nstrand = wire_list[i].concentric_neutral_nstrand

                if wire_list[i].concentric_neutral_diameter is None:
                    concentric_neutral_diameter_meters = 1 / 1000.0 # 1 mm strand diameter
                    logger.warning('Warning - using default concentric_neutral_diameter of '+str(concentric_neutral_diameter_meters))
                else:
                    concentric_neutral_diameter_meters = wire_list[i].concentric_neutral_diameter

                if wire_list[i].concentric_neutral_outside_diameter is None:
                    concentric_neutral_outside_diameter_meters = 25.93 /1000.0 
                    logger.warning('Warning - using default concentric_neutral_outside_diameter of '+str(concentric_neutral_outside_diameter_meters))
                else:
                    concentric_neutral_outside_diameter_meters = wire_list[i].concentric_neutral_outside_diameter
                ...
                if wire_list[i].concentric_neutral_resistance is None:
                    ...
                    logger.warning('Warning - using default concentric_neutral_resistance of '+str(concentric_neutral_resistance_meters))
                else:
                    concentric_neutral_resistance_meters = wire_list[i].concentric_neutral_resistance

                if wire_list[i].resistance is None:
                    resistance_meters =0.000269 #Aluminium wire resistivity per meter for 4/0 wire (Nexans)
                    logger.warning('Warning - using default resistance of '+str(resistance_meters))
                else:
                    resistance_meters = wire_list[i].resistance

                if wire_list[i].gmr is None:
                    gmr_meters = 13.26/2.0/1000.0 *0.7788 #using 4/0 wire diameter of 13.26
                    logger.warning('Warning - using default gmr of '+str(gmr_meters))
                else:
                    gmr_meters = wire_list[i].gmr

                radius_meters = (concentric_neutral_outside_diameter_meters - concentric_neutral_diameter_meters)/2.0
                radius = self.convert_from_meters(radius_meters,'ft')
                concentric_neutral_gmr = self.convert_from_meters(concentric_neutral_gmr_meters, 'ft')
                gmr_neutral = (concentric_neutral_gmr*concentric_neutral_nstrand*(radius**(concentric_neutral_nstrand-1)))**(1/concentric_neutral_nstrand)
                r_neutral = self.convert_from_meters(concentric_neutral_resistance_meters,'mi',inverse=True)/float(concentric_neutral_nstrand)
                gmrs.append(self.convert_from_meters(gmr_meters,'ft'))
                rs.append(self.convert_from_meters(resistance_meters,'ft',inverse=True))
            ...
            distance_matrix_cn = [[None for i in range(2*nphases)] for j in range(2*nphases)]
            for i in range(nphases):
                for j in range(nphases):
                    distance_matrix_cn[i][j] = distance_matrix_feet[i][j]
                    distance_matrix_cn[i+nphases][j+nphases] = distance_matrix_feet[i][j]
            ...
            impedance_matrix_imperial = self.get_primitive_impedance_matrix(np.array(distance_matrix_cn),gmrs_cn,rs_cn)
            # Now Kron reduce out the nphases neutrals
            neutrals = []
            for i in range(nphases):
                neutrals.append(i+nphases)
            impedance_matrix_imperial = self.kron_reduction(impedance_matrix_imperial,neutrals=neutrals)
