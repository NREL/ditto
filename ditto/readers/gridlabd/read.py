...
'''
            ...
            for w in conductors:
                ...
                for non_rotated in tmp_map[w.phase]:
                    ...
                    if has_earth:
                        ...
                        rotation = np.array([[math.cos(math.pi/2),-1*math.sin(math.pi/2)],[math.sin(math.pi/2),math.cos(math.pi/2)]])
                        final = rotation.dot(non_rotated)
                        final[1] = final[1]+default_height
                        break
                w.X = final[0]/3.28084
            ...
            for w in conductors:
                ...
                if index == max_from:
                    ...
                    if seen_one:
                        ...
                        if (x-first_x)**2 + (y-first_y)**2 != distances[index][first_index]**2:
                            x = x*-1
                    else:
                        seen_one = True
                        first_x = x
    ...
    def compute_secondary_matrix(self,wire_list,freq=60,resistivity=100,kron_reduce=True):
        ...
        for i in range(len(wire_list)):
            for j in range(len(wire_list)):
                if i==j:
                    z=0
                    if wire_list[i].resistance is not None and wire_list[i].gmr is not None:
                        z = complex(wire_list[i].resistance + 0.00158836*freq ,0.00202237*freq*(math.log(1/wire_list[i].gmr)+7.6786+0.5*math.log(resistivity/freq)))
                    else:
                    ...
                    z=0
                    if wire_list[i].phase is not None and wire_list[j].phase is not None and distances_mapped:
                        if wire_list[i].phase == 'N' or wire_list[j].phase == 'N':
                            z = complex(0.00158836*freq, 0.00202237*freq*(math.log(1/d1n)+7.6786+0.5*math.log(resistivity/freq)))
                        else:
                            z = complex(0.00158836*freq, 0.00202237*freq*(math.log(1/d12)+7.6786+0.5*math.log(resistivity/freq)))
        ...
        if kron_reduce:
            ...
            for i in range(2):
                for j in range(2):
                    kron_matrix[i][j] = matrix[i][j]-matrix[i][2]*1/matrix[2][2]*matrix[2][j]
    ...
    def compute_matrix(self,wire_list,freq=60,resistivity=100,kron_reduce=True):
        ...
        for i in range(len(wire_list)):
            ...
            for j in range(len(wire_list)):
                if i==j:
                    ...
                    z=0
                    if wire_list[i].X is not None and wire_list[i].Y is not None and wire_list[j].X is not None and wire_list[j].Y is not None:
                        x1 = wire_list[i].X
                        x2 = wire_list[j].X
                        y1 = wire_list[i].Y
                        y2 = wire_list[j].Y
        ...
        if kron_reduce and has_neutral:
            ...
            for i in range(3):
                for j in range(3):
                    kron_matrix[i][j] = matrix[i][j]-matrix[i][3]*1/matrix[3][3]*matrix[3][j]
    ...
    def parse(self, model, origin_datetime = '2017 Jun 1 2:00PM'):
        ...
        for row in all_rows:
            ...
            if len(entries)>0 and entries[0]=='object':
                if curr_object is None:
                    obj = entries[1].split(':')
                    obj_class = obj[0]
                    if obj_class == "house" or obj_class == 'solar' or obj_class == 'inverter'  or obj_class == "waterheater" or obj_class == 'climate' or obj_class == 'ZIPload' or obj_class == 'tape.recorder' or obj_class == 'player' or obj_class == 'tape.collector' or obj_class == 'tape.group_recorder' or obj_class == 'recorder':
                        continue
        ...
        for obj_name,obj in self.all_gld_objects.items():
            ...
            if obj_type == 'node' or obj_type =='meter':
                ...
                try:
                    phases = []
                    for i in obj['phases'][0:-1]:#Ignore the 'N' at the end and just say if A,B or C
            ...
            if obj_type == 'triplex_node' or obj_type =='triplex_meter':
                ...
                try:
                    phases = []
                    cnt = 0
                    for i in obj['phases'][1:-1]: #Ignore the 'S' at the end and just say if A,B or C
                        # With lists of traitlets, the strings aren't automatically cast
                        cnt=cnt+1
                        ...
                        if config_name == config_name1:
                            try:
                                ...
                                if conn == 5 or conn == 'SINGLE_PHASE_CENTER_TAPPED':
                                    num_windings = 3
                                    winding2.phase_windings[0].phase='A' #Assume that only one phase from A/B/C was assigned to the winding. TODO replace with 2 to represent secondaries.

                                    pw3 = PhaseWinding(model)
                                    pw3.phase = 'B' #TODO replace with 2 to represent secondaries.
                            ...
                            try:
                                ...
                                if num_windings == 3:
                                    winding1.resistance = resistance/2.0
                                    winding2.resistance = resistance #Using power flow approximation from "Electric Power Distribution Handbook" by Short page 188
                            ...
                            try:
                                ...
                                reactances.append(reactance1) #TODO: Change documentation to reflect that we aren't indicating the from-to relation in reactances.
                                #reactances.append((0,1,reactance1))
                                if num_windings == 3: #TODO: Change documentation to reflect that we aren't indicating the from-to relation in reactances.
                                ...
                                if not failed_reactance: # Should only fail if there are three windings in the system
                                    reactance = float(config['reactance'])
                                    reactances[0]=0.8*reactance
                                    reactances.append(0.4*reactance) #Using power flow approximation from "Electric Power Distribution Handbook" by Short page 188 of transformer with no center tap
                ...
                for phase in phases:
                    if phase == 'A':
                        ...
                        try:
                            ...
                            if p !=0: # Assume that unless ZIP is specifically specified only one of constant_power, constant_current and constant_impedance is used. A real part of 0 means that I or Z is being used instead.
                                phaseload.p = p
                                phaseload.q = q
                        ...
                        try:
                            ...
                            complex_voltage = complex(obj['voltage_A']) # Needed to compute the power
                            if complex_impedance.real !=0: # Assume that unless ZIP is specifically specified only one of constant_power, constant_current and constant_impedance is used. A real part of 0 means that I or P is being used instead.
                                p=(((complex_voltage.real**2+complex_voltage.imag**2))/complex_impedance.conjugate()).real
                                q=(((complex_voltage.real**2+complex_voltage.imag**2))/complex_impedance.conjugate()).imag
                                phaseload.p = p
                                phaseload.q = q
                                phaseload.use_zip = 0
                        ...
                        try:
                            ...
                            if complex_current.real !=0: # Assume that unless ZIP is specifically specified only one of constant_power, constant_current and constant_impedance is used. A real part of 0 means that Z or P is being used instead.
                        ...
                        try:
                            ...
                            if data[0] in all_schedules:
                                phaseload.p = float(all_schedules[data[0]])*float(data[1])
                            if data[1] in all_schedules:
                                phaseload.p = float(all_schedules[data[1]])*float(data[0])
                        ...
                        try:
                            ...
                            complex_voltage = complex(obj['voltage_B']) # Needed to compute the power
                            if complex_impedance.real !=0: # Assume that unless ZIP is specifically specified only one of constant_power, constant_current and constant_impedance is used. A real part of 0 means that I or P is being used instead.
                                p=(((complex_voltage.real**2+complex_voltage.imag**2))/complex_impedance.conjugate()).real
                                q=(((complex_voltage.real**2+complex_voltage.imag**2))/complex_impedance.conjugate()).imag
                                phaseload.p = p
                        ...
                        try:
                            ...
                            complex_voltage = complex(obj['voltage_C']) # Needed to compute the power
                            if complex_impedance.real !=0: # Assume that unless ZIP is specifically specified only one of constant_power, constant_current and constant_impedance is used. A real part of 0 means that I or P is being used instead.
                                p=(((complex_voltage.real**2+complex_voltage.imag**2))/complex_impedance.conjugate()).real
                                q=(((complex_voltage.real**2+complex_voltage.imag**2))/complex_impedance.conjugate()).imag
                                phaseload.p = p
            ...
            if obj_type == 'overhead_line':
                ...
                try:
                    ...
                    for api_wire in conductors:
                        ...
                        try:
                            api_wire.diameter = float(conductor['diameter']) #i.e. the same for all the wires.
                        except AttributeError:
                            pass
                        try:
                        ...
                        try:
                            ...
                            pass
                        try:
                            api_wire.ampacity = float(conductor['rating.summer.continuous'])
                        except AttributeError:
                            pass
                        try:
                            api_wire.ampacity_emergency = float(conductor['rating.summer.emergency'])
                        except AttributeError:
                            pass
                ...
                for api_wire in conductors:
                    try:
                        if api_wire.diameter is not None:
                            api_wire.diameter = api_wire.diameter/39.3701 #i.e. the same for all the wires.
                    except AttributeError:
                        pass
                    try:
            ...
            if obj_type == 'triplex_line':
                ...
                try:
                    ...
                    try:
                        api_wire.insulation_thickness = float(config['insulation_thickness'])/39.3701
                    except AttributeError:
                        pass
                    ...
                    for api_wire in conductors:
                        ...
                        conductor = self.all_gld_objects[cond_name]
                        try:
                            api_wire.gmr = float(conductor['geometric_mean_radius'])/3.28084
                        except AttributeError:
                            pass
                        try:
                            if api_line.length is not None:
                                api_wire.resistance = float(conductor['resistance'])*api_line.length/1609.34 # since we converted the length to m already
                        except AttributeError:
                            pass
            ...
            if obj_type == 'underground_line':
                ...
                try:
                    ...
                    for api_wire in conductors:
                        ...
                        try:
                            api_wire.concentric_neutral_gmr = float(conductor['neutral_gmr'])
                        except AttributeError:
                            pass
                        # if concentric resistances is not None set it for underground cables
                        try:
                            api_wire.concentric_neutral_resistance = float(conductor['neutral_resistance'])
                        except AttributeError:
                            pass
                    ...
                    if spacing_name is not None:
                        ...
                        if n_entries == 3: # make longest side on bottom to form a triangle
                            ...
                            for i in range(n_entries):
                                if i != first and i != last and distances[i][first] != -1:
                                    middle = i
                            heron_s = total/2.0
                            heron_area = heron_s
                            for i in range(n_entries):
                            ...
                            for w in conductors:
                                if w.phase == lookup[first]:
                                    ...
                                    w.Y = -6+ height
                                    w.X = math.sqrt( distances[middle][first]**2 - height **2)
                        ...
                        if n_entries == 4: # make longest side on bottom to form a triangle
                            ...
                            for w in conductors:
                                ...
                                if i != max_to and i!= max_from:
                                    ...
                                    if not seen_one:
                                        w.Y = -6+height
                                        w.X = math.sqrt(distances[i][max_from]**2 - height**2)
                                        seen_one = True
                                        first_x = w.X
                                        first_y = w.Y
                                        first_i = i
                                        ...
                                        w.Y = -6+height
                                        w.X = math.sqrt(distances[i][max_from]**2 - height**2)
                                        if (w.X- first_x) **2 + (w.Y - first_y)**2 != distances[i][first_i]**2:
                                            w.Y = -6-height
                                elif i == max_from:
                                    w.X = 0
                                    w.Y = -6
                ...
                for api_wire in conductors:
                    ...
                    try:
                        if api_wire.concentric_neutral_gmr is not None:
                            api_wire.concentric_neutral_gmr = api_wire.concentric_neutral_gmr/3.28084
                    except AttributeError:
                        pass
                    try:
                        if api_wire.resistance is not None:
                    ...
                    try:
                        if api_wire.concentric_neutral_resistance is not None:
                            api_wire.concentric_neutral_resistance = api_wire.concentric_neutral_resistance/1609.34 # In Ohms per meter
                    except AttributeError:
                        pass
            ...
            if obj_type == 'capacitor':
                ...
                try:
                    ...
                    pc.phase = 'A'
                    pc.var = varA * 1000000
                    phase_capacitors.append(pc) # in case there is no switching attribute
                        ...
                        if config_name == config_name1:
                            ...
                            for tap_phase in ['A','B','C']:
                                try:
                                    ...
                                    if winding2.phase_windings is None: #i.e. no phases were listed even though they are there. Should only need to check winding2 (not both windings) since the phases are populated at the same time.
                                        winding1.phase_windings = []
                                        winding2.phase_windings = []

                                    index = None
                                    for i in range(len(winding2.phase_windings)):
                                        if winding2.phase_windings[i].phase == tap_phase:
                                            index = i
                                            break
                                    if index is None:
                                        pw1 = PhaseWinding(model)
                                    ...
                                    winding2.phase_windings[index].tap_position = int(tap)
                            ...
                            for r_comp_phase in ['A','B','C']:
                                try:
                                    r_comp = config['compensator_r_setting_%s'%r_comp_phase]
                                    if winding2.phase_windings is None: #i.e. no phases were listed even though they are there. Should only need to check winding2 (not both windings) since the phases are populated at the same time.
                                        winding1.phase_windings = []
                                        winding2.phase_windings = []
                                    ...
                                    index = None
                                    for i in range(len(winding2.phase_windings)):
                                        if winding2.phase_windings[i].phase == r_comp_phase:
                                            index = i
                                            break
                                    if index is None:
                                        pw1 = PhaseWinding(model)
                                        ...
                                        winding2.phase_windings.append(pw2) #Add the phase in for winding 1 as well
                                        index = len(windings2.phase_windings)-1


                                    winding2.phase_windings[index].compensator_r = float(r_comp)
                            ...
                            for x_comp_phase in ['A','B','C']:
                                try:
                                    x_comp = config['compensator_x_setting_%s'%x_comp_phase]
                                    if winding2.phase_windings is None: #i.e. no phases were listed even though they are there. Should only need to check winding2 (not both windings) since the phases are populated at the same time.
                                        winding1.phase_windings = []
                                        winding2.phase_windings = []
                                    ...
                                    index = None
                                    for i in range(len(winding2.phase_windings)):
                                        if winding2.phase_windings[i].phase == x_comp_phase:
                                            index = i
                                            break
                                    if index is None:
                                        pw1 = PhaseWinding(model)
                                    ...
                                    winding2.phase_windings[index].compensator_x = float(x_comp)
                            ...
                            try:
                                api_regulator.pt_ratio = float(config['power_transducer_ratio'])
                            except AttributeError:
                                pass

                            try:
                                api_regulator.ct_ratio = float(config['current_transducer_ratio'])
                            except AttributeError:
                                pass

                            try:
                                #wire_map = {'A':1,'B':2,'C':3} #Only take one phase (GLD seems to have 3 sometimes)
                                api_regulator.pt_phase = config['PT_phase'].strip('"')[0]#wire_map[config['PT_phase'].strip('"')[0]]
                            except AttributeError:
                                pass
