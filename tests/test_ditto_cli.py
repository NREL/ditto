...
def test_opendss_to_gridlabd_cli():
    ...
    p = subprocess.Popen(
        shlex.split(
            """ ditto-cli convert --from="opendss" --to="gridlabd" --input="./data/small_cases/opendss/ieee_13node/master.dss" --output="{}" """.format(
                output_path.name
            ).strip()
        )
    )
...
def test_gridlabd_to_opendss_cli():
    ...
    p = subprocess.Popen(
        shlex.split(
            """ ditto-cli convert --from="gridlabd" --to="opendss" --input="./data/small_cases/gridlabd/4node.glm" --output="{}" """.format(
                output_path.name
            ).strip()
        )
    )
...
def test_opendss_to_ephasor_cli():
    ...
    p = subprocess.Popen(
        shlex.split(
            """ditto-cli convert --from="opendss" --to="ephasor" --input="./data/small_cases/opendss/ieee_13node/master.dss" --output="{}" """.format(
                output_path.name
            ).strip()
        )
    )
