...
def verify_component(component_dict, model_obj, component, to_append):
    ...
    if (component == 'loads') or (component == 'phase_loads'):
        ...
        names = list(component_dict['transformers']['Transformer.name'])
        obj_names = [name for name in names]
    elif component == 'wires':
        names = list(component_dict['lines']['Line.name'])
        obj_names = [name for name in names]
        pass
    else:
        obj_names = names
    csv_set = set(names)
    model_set = set(model_obj.model_names.keys())
    if component == 'loads':
        ...
        if component == 'windings':
            ...
            sub_obj = len(model_obj.model_names['t1'].windings)
            for name in obj_names:
        ...
        assert csv_set.issubset(model_set)
...
def verify_properties(component_dict, model_obj, component, to_append):
    ...
    if component == 'phase_loads':
        ...
        loads = [key for key in model_obj.model_names.keys() if 'load' in key]
        ...
        csv_set.remove(to_append + 'name')
        transformers = component_dict['transformers']['Transformer.name']
        ...
        if component == 'loads':
            names = ['load_' + name for name in names]
        for name in names:
            model_set = model_obj.model_names[name].trait_names()
            ...
            if phase_count > 0:
                if component == 'loads':
                    ...
                    assert phase_count == len(model_obj.model_names[names[0]].phases)
            ...
            for t in csv_set:
                if 'phase' in t:
                    ...
                    try:
                        ...
                        if str(e) == 'assert 12.47 == 12470.0' or 'assert 4.16 == 4160.0':
                            raise
            if phase_count > 0:
